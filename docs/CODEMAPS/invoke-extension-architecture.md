...
### Фаза 6 — Форма → Очередь / Отправка (UI-интеграция)

**Статус:** Реализовано (2026-06-03)
**Назначение:** Пользовательский интерфейс для постановки invoke в очередь и отправки в другую базу непосредственно с формы шаблона `ОДМ_МетодыИнтеграции`.

#### Component Diagram

```
Форма элемента ОДМ_МетодыИнтеграции (Form.Module.bsl)
├── Кнопка «В очередь»
│   └── ПоставитьВОчередьНаСервере(Код, JSON)
│       └── СоздатьЗадачуInvokeВОчередь()          ← НОВАЯ функция
│           └── Регистр: Метод="invoke", Статус=Подготовлен
│               └── РЗ: ОбработкаЗаданий_Invoke → ВыполнитьПоШаблону()
│
├── Поле «База-получатель» (СправочникСсылка.ОДМ_БазыДанных)
│
└── Кнопка «Отправить»
    └── ОтправитьВБазуНаСервере(Код, JSON, БазаСсылка)
        └── СоздатьЗадачуInvokeОтправкиПоБазе()    ← НОВАЯ функция
            ├── ЗначенияРеквизитовОбъекта(БазаСсылка, "ИмяПубликации, СерверПубликации, ...")
            └── СоздатьЗадачуInvokeОтправки(ИмяПубликации, Код, Параметры, ДопПараметры)
                └── Регистр: Метод="invokeОтправить", Статус=Подготовлен
                    └── РЗ: ОбработкаЗаданий_InvokeОтправить → HTTP /invoke
```

#### Новые API-функции

| Функция | Модуль | Сигнатура |
|---------|--------|-----------|
| `СоздатьЗадачуInvokeВОчередь` | `ОДМ_ОбменДанными` | `(КодШаблона, Параметры) → Строка(GUID)` |
| `СоздатьЗадачуInvokeОтправкиПоБазе` | `ОДМ_ОбменДанными` | `(БазаСсылка, КодШаблона, Параметры) → Строка(GUID)` |

#### Новые элементы формы

| Элемент | Тип | DataPath |
|---------|-----|----------|
| `БазаПолучатель` | Поле ввода + выбор | `БазаПолучатель` (реквизит формы: `CatalogRef.ОДМ_БазыДанных`) |
| `ПоставитьВОчередь` | Кнопка команды | → `ПоставитьВОчередьНаСервере()` |
| `ОтправитьВБазу` | Кнопка команды | → `ОтправитьВБазуНаСервере()` |

#### Сценарий: Пользователь → Очередь

```
1. Открыть форму шаблона → вкладка «Предпросмотр JSON»
2. Нажать «Сформировать пример запроса» → JSON в поле
3. Отредактировать значения-заглушки на реальные данные
4. Нажать «В очередь»
   → ДесериализоватьJSON → СоздатьЗадачуInvokeВОчередь(Код, Параметры)
   → Оповещение: «Задача поставлена в очередь: GUID»
5. Через ~1 мин РЗ ОДМ_ОбработкаЗадач:
   → ОбработкаЗаданий_Invoke → ВыполнитьПоШаблону локально
```

#### Сценарий: Пользователь → Отправка в другую базу

```
1–3. Те же шаги
4. Выбрать базу в поле «База-получатель»
5. Нажать «Отправить»
   → ЗначенияРеквизитовОбъекта(БазаСсылка) → читает ИмяПубликации, СерверПубликации
   → СоздатьЗадачуInvokeОтправкиПоБазе(БазаСсылка, Код, Параметры)
   → Оповещение: «Задача отправки создана: GUID»
6. Через ~1 мин РЗ:
   → ОбработкаЗаданий_InvokeОтправить → ОтправитьInvokeНаСервер()
   → HTTP POST /ИмяПубликации/hs/MRS_xchg/invoke → выполнение на получателе
```

#### Trade-off: почему не динамическая генерация полей ввода

| Аспект | |
|---|---|
| **Выбор** | Не генерировать поля ввода динамически по ТЧ `ВходныеПараметры`. Вместо этого пользователь редактирует сырой JSON в поле `ПримерJSONЗапроса`. |
| **Pros** | Минимальные изменения формы (только 1 новое поле + 2 кнопки). Не нужно управлять жизненным циклом динамических элементов. |
| **Cons** | Пользователь работает с сырым JSON — выше риск ошибок формата. |
| **Alternatives** | Динамическая генерация полей по типам из ТЧ — отложена на будущую итерацию. |

#### Trade-off: мост к `ОДМ_БазыДанных` через отдельную функцию

| Аспект | |
|---|---|
| **Выбор** | `СоздатьЗадачуInvokeОтправкиПоБазе` — отдельная функция-мост, а не расширение сигнатуры `СоздатьЗадачуInvokeОтправки`. |
| **Pros** | SRP: одна функция читает справочник, другая создаёт задачу. Не ломает существующий API. |
| **Cons** | Две функции вместо одной универсальной. |
| **Alternatives** | Принимать `СправочникСсылка` напрямую в `СоздатьЗадачуInvokeОтправки` — отброшено: нарушает сигнатуру, требует изменения всех существующих вызовов. |
...
### Форма элемента — конвенция именования и зеркальные реквизиты (2026-06-10)

**Контекст:** форма элемента `Справочник.ОДМ_МетодыИнтеграции` использует JSON-редактор (`ОДМ_УправлениеФормами_Сервер.ИнициализацияРедактораJSON_ПриСозданииНаСервере`), который заменяет обычные поля ввода на HTML-редакторы с подсветкой синтаксиса.

**Зеркальные реквизиты `_ОДМ_*`:**
- JSON-редактор **программно** создаёт form-атрибуты `_ОДМ_Шаблон`, `_ОДМ_ПримерJSONЗапроса`, `_ОДМ_ПримерJSONОтвета` (тип Строка(1000)) через `Форма.ИзменитьРеквизиты`
- Оригинальные поля формы скрываются (`Видимость = Ложь`), вместо них отображаются HTML-редакторы, привязанные к зеркальным реквизитам
- Поток данных: пользователь → HTML-редактор → `_ОДМ_*` → `ПередЗаписьюНаСервере` → `Объект.*`

**Конвенция именования в модуле формы:**
- Обычные form-атрибуты — **без префикса**: `ТаблицаПараметров` (не `_ОДМ_ТаблицаПараметров`)
- Зеркальные реквизиты JSON-редактора — с префиксом `_ОДМ_*`: только в sync-процедурах (`СинхронизироватьЗеркалаИзОбъекта`, `ПередЗаписьюНаСервере`) и в client-обработчиках кнопок (только чтение `_ОДМ_ПримерJSONЗапроса`, потому что HTML-редактор пишет именно в зеркало)
- Серверные методы, формирующие данные (`СформироватьПримерЗапросаНаСервере`, `СобратьJSONНаСервере`) — пишут в `Объект.ПримерJSONЗапроса` + синхронизируют зеркало: `_ОДМ_ПримерJSONЗапроса = Объект.ПримерJSONЗапроса`

**Файлы:**
- `Catalogs/ОДМ_МетодыИнтеграции/Forms/ФормаЭлемента/Ext/Form.xml` — form-атрибут `ТаблицаПараметров` (ValueTable, id=116)
- `Catalogs/ОДМ_МетодыИнтеграции/Forms/ФормаЭлемента/Ext/Form/Module.bsl` — модуль формы
- `CommonModules/ОДМ_УправлениеФормами_Сервер/Ext/Module.bsl` — создание зеркальных реквизитов
- `CommonModules/ОДМ_УправлениеФормами_Клиент/Ext/Module.bsl` — инициализация HTML-редактора на клиенте
