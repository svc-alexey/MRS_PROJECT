...
**Дата реализации Phase 1-5:** 2026-05-29
**Дата реализации Phase 6:** 2026-06-03
...
6. **Форма → Очередь / Отправка** — с формы элемента справочника можно поставить invoke в очередь (локально) или отправить в выбранную базу из `ОДМ_БазыДанных`
...
- [x] **F11.** Кнопка «В очередь» на форме шаблона создаёт задачу `Метод = "invoke"` в регистре
- [x] **F12.** Кнопка «Отправить» с выбранной базой создаёт задачу `Метод = "invokeОтправить"` в регистре
- [x] **F13.** `СоздатьЗадачуInvokeОтправкиПоБазе` читает параметры подключения из `ОДМ_БазыДанных` через `ЗначенияРеквизитовОбъекта`
- [x] **F14.** Обе кнопки валидируют заполненность `ПримерJSONЗапроса`

---

### Phase 6: Форма → Очередь / Отправка (UI) ✅

> **Дата реализации:** 2026-06-03
> **Причина:** Phases 1-5 создали инфраструктуру (очередь, retry, исходящий invoke), но не было пользовательского интерфейса для их использования с формы шаблона.

#### Step 6.1: `СоздатьЗадачуInvokeВОчередь()` — новый API
- **Файл:** `CommonModules/ОДМ_ОбменДанными/Ext/Module.bsl`
- **Сигнатура:** `СоздатьЗадачуInvokeВОчередь(КодШаблона, Параметры) → Строка(GUID)`
- **Логика:** 1:1 копия паттерна `СоздатьЗадачуInvokeОтправки`, но с `Метод = "invoke"` и без `БазаПолучатель`

#### Step 6.2: `СоздатьЗадачуInvokeОтправкиПоБазе()` — мост к справочнику
- **Файл:** `CommonModules/ОДМ_ОбменДанными/Ext/Module.bsl`
- **Сигнатура:** `СоздатьЗадачуInvokeОтправкиПоБазе(БазаСсылка, КодШаблона, Параметры) → Строка(GUID)`
- **Логика:** Читает `ИмяПубликации, СерверПубликации, Логин, Пароль` через `ЗначенияРеквизитовОбъекта()`, делегирует `СоздатьЗадачуInvokeОтправки()`

#### Step 6.3: Форма — поле `БазаПолучатель` + 2 кнопки
- **Файлы:** `Form.xml` + `Form.Module.bsl`
- **Элементы:** поле выбора базы (СправочникСсылка.ОДМ_БазыДанных), кнопки «В очередь» и «Отправить»
- **Валидация:** кнопки активны только при непустом `ПримерJSONЗапроса`; «Отправить» требует заполненного `БазаПолучатель`

**Проверка Phase 6:**
1. `syntaxcheck` для `ОДМ_ОбменДанными.Ext.Module.bsl` — без ошибок
2. `syntaxcheck` для модуля формы — без ошибок
3. `Form.xml` — уникальность ID, well-formedness — проверено
...
| 6.1 | `СоздатьЗадачуInvokeВОчередь()` | `ОДМ_ОбменДанными` | +25 | `syntaxcheck` |
| 6.2 | `СоздатьЗадачуInvokeОтправкиПоБазе()` | `ОДМ_ОбменДанными` | +30 | `syntaxcheck` |
| 6.3 | Форма: поле + 2 кнопки | `ФормаЭлемента` + модуль | +70 XML, +60 BSL | `syntaxcheck` + XML check |

**Всего:** ~410 строк кода, ~150 строк XML, 15 шагов в 5 фазах (исходный план). +115 строк, +70 XML, 3 шага в 1 фазе (Phase 6).
