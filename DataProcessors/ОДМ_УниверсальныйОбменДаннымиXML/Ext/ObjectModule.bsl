...
Функция ПолучитьАлгоритмыОбработчика(Обработчик)
	...
	Для Индекс = 1 По СтрЧислоСтрок(Обработчик) Цикл
		...
		Если Лев(СтрокаОбработчика, 2) = "//" Тогда //Строка закомментирована, ее пропускаем..
			Продолжить;
		КонецЕсли;
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если      ИмяУзла = "Запись" Тогда
			...
			Если Запись = Неопределено Тогда
				одПропустить(ФайлОбмена);
			Иначе
				УстановитьСвойстваЗаписи(Объект, Запись, ИнформацияОТипах, ПараметрыОбъекта, ИмяТабличнойЧасти, НомерЗаписи, ДанныеПоискаПоТЧ, КопияТЧДляПоиска);
			КонецЕсли;
...
Процедура ЗагрузитьДвижения(Объект, Имя, Очистить, ОбщаяИнформацияОТипеДокумента, НужноЗаписатьОбъект, 
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если      ИмяУзла = "Запись" Тогда
			...
			НужноЗаписатьОбъект = Истина;
			УстановитьСвойстваЗаписи(Объект, Запись, ИнформацияОТипах, ПараметрыОбъекта, ИмяДвижений, НомерЗаписи, ДанныеПоискаПоТЧ, КопияТЧДляПоиска);
...
Процедура _ВыгрузитьСубконто(Источник,
							Приемник,
							ВходящиеДанные,
							ИсходящиеДанные,
							ПКО,
...
Функция ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПодстановкиВПКО = Неопределено, Свойства = Неопределено)
	...
	//--- { НеСА 2015-02-18 
	Если Правило.НастройкиПостроителя <> Неопределено 
		И ТипЗнч(Правило.НастройкиПостроителя) <> Тип("ОтборКомпоновкиДанных") Тогда
	...
	Если ТипЗнч(Правило.НастройкиПостроителя) = Тип("ОтборКомпоновкиДанных") Тогда
		Выборка = ВыборкаПоКомпоновщику(Правило,ПостроительОтчета.Текст);
	Иначе
		ПостроительОтчета.Выполнить();
		Выборка = ПостроительОтчета.Результат.Выбрать();
	КонецЕсли;
...
Функция ВыборкаПоКомпоновщику(Правило,ТекстЗапроса)
	/////////////////////////////////////////////////////////
	...
	//СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Отбор = Правило.НастройкиПостроителя;
	Для каждого Настройка из Правило.НастройкиПостроителя.Элементы Цикл
		СкопироватьНастройку(СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Отбор.Элементы,Настройка)
	КонецЦикла;
	...
	ПараметрДанных = СхемаКомпоновкиДанных.НастройкиПоУмолчанию.ПараметрыДанных.Элементы.Добавить();
	ПараметрДанных.Параметр = Новый ПараметрКомпоновкиДанных("ДатаНачала");
	ПараметрДанных.Использование = Истина;
	ПараметрДанных.Значение = ДатаНачала;
	
	ПараметрДанных = СхемаКомпоновкиДанных.НастройкиПоУмолчанию.ПараметрыДанных.Элементы.Добавить();
	ПараметрДанных.Параметр = Новый ПараметрКомпоновкиДанных("ДатаОкончания");
	ПараметрДанных.Использование = Истина;
	ПараметрДанных.Значение = ДатаОкончания;
	...
	Запрос = Новый ЗАпрос;
	Запрос.Текст = 
	"ВЫБРАТЬ * ПОМЕСТИТЬ Врем_ТЗ ИЗ &ТЗ КАК ТЗ;
	|ВЫБРАТЬ * ИЗ Врем_ТЗ КАК Врем_ТЗ";
...
Функция ЭтоВнешняяОбработка()
	...
	Возврат ?(Найти(ИмяФайлаВнешнейОбработкиОбработчиковСобытий, ".") <> 0, Истина, Ложь);
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	Попытка
		Если Ложь Тогда
			ДополнительныеОтчетыИОбработки = "";	//иначе не скомпилируется, где нет модуля
		КонецЕсли;
	...
	ПараметрыРегистрации.Версия = "0.0.5";
	ПараметрыРегистрации.Вставить("ДатаВерсии", Дата(2016, 11, 25));
	
	ПараметрыРегистрации.Наименование = ЭтотОбъект.Метаданные().Синоним;
	
	Если ПараметрыРегистрации.Свойство("Команды") Тогда
