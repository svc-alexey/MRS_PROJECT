...
Процедура ПоказатьОбработчикиСобытийВОкне()
	...
	//-> 2021-04-01
	Иначе //попробуем его на сервере получить
		ТекстовыйДокумент = ПрочитатьФайлНаСервере(Объект.ИмяВременногоФайлаОбработчиковСобытий);
		Если НЕ ТекстовыйДокумент = Неопределено Тогда
			ТекстовыйДокумент.Показать(Нстр("ru = 'Модуль отладки обработчиков'"));
		КонецЕсли;
	//2021-04-01 <-
	КонецЕсли;
...
Функция ПрочитатьФайлНаСервере(ИмяФайлаОбработчиков)
	
	Перем ТекстовыйДокумент;
	
	ФайлОбработчиков = Новый Файл(ИмяФайлаОбработчиков);
	Если ФайлОбработчиков.Существует() И ФайлОбработчиков.Размер() <> 0 Тогда
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.Прочитать(ИмяФайлаОбработчиков);
	КонецЕсли;
