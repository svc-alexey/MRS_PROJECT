...
Процедура ПриОткрытии(Отказ)
	...
	ПодключитьОбработчикОжидания("ПроверкаАктуальностиФайлаПравилОбмена", 10);

КонецПроцедуры

&НаКлиенте
Процедура ПроверкаАктуальностиФайлаПравилОбмена()
	
	ФайлИзменился = Ложь;
	
	Если ЭтоКлиент Тогда
		ФайлИзменился = ФайлИзменился(ИмяФайлаПравил, ВремяИзмененияФайлаПравил);
	Иначе
		ФайлИзменился = ФайлПравилИзменилсяНаСервере(ИмяФайлаПравил, ВремяИзмененияФайлаПравил);
	КонецЕсли;
	
	Картинка = ?(ФайлИзменился, БиблиотекаКартинок.Обновить, БиблиотекаКартинок.СинтаксическийКонтроль);
	Шрифт    = ?(ФайлИзменился, Новый Шрифт(, , Истина), Новый Шрифт);
	
	Элементы.ФормаЗагрузитьПравила.Картинка = Картинка;
	Элементы.ФормаЗагрузитьПравила.Шрифт = Шрифт;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ФайлПравилИзменилсяНаСервере(Знач ИмяФайлаПравил, Знач ВремяИзмененияФайлаПравил)
...
Функция ФайлИзменился(Знач ИмяФайлаПравил, Знач ВремяИзмененияФайлаПравил)
	...
	Попытка
		...
		Если Файл.Существует() Тогда
			...
			РезультатФункции = НЕ (ВремяИзмененияФайлаПравил = ВремяИзменения);
		КонецЕсли;
	Исключение
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если ЭтоКлиент Тогда
		Интерактивно = Истина;
		Если НЕ ПустаяСтрока(ИмяФайлаПравил) Тогда
			ФайлПравил = Новый Файл(ИмяФайлаПравил);
			Интерактивно = НЕ ФайлПравил.Существует();
			ВремяИзмененияФайлаПравил = ФайлПравил.ПолучитьВремяИзменения();
		КонецЕсли;
				
		ОписаниеОповещения = Новый ОписаниеОповещения("ПрочитатьПравилаОбменаЗавершение", ЭтотОбъект);
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайлаПравилВХранилище, ИмяФайлаПравил, Интерактивно, УникальныйИдентификатор);
	Иначе
...
Процедура ПрочитатьПравилаОбменаЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		ИмяФайлаПравил = ВыбранноеИмяФайла;
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления()
	...
	Для каждого ОбъектМД Из Метаданные.РегистрыСведений Цикл
		...
		// СВР 2017-10-23 такого права нет и платформа стала тут спотыкаться... заремим
		//Если Не ПравоДоступа("Удаление", ОбъектМД) Тогда
		//	Продолжить;
		//КонецЕсли;
...
Процедура ВыполнитьЗагрузкуИзФормы()
	...
	Если ЭтоКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещения, АдресФайла, ИмяФайлаОбмена, , УникальныйИдентификатор);
...
Функция ИмяФайлаНаСервереИлиКлиенте(ИмяРеквизита ,Знач АдресФайла = "", Знач ИмяФайлаДляРасширения = ".xml",
	...
	Если ЭтоКлиент Тогда
		...
		Иначе
			//2021-04-15
			Если ПустаяСтрока(ИмяФайлаДляРасширения) Тогда
				ИмяФайлаДляРасширения = ИмяРеквизита;
			КонецЕсли;
			//2021-04-15
...
Процедура ВыполнитьВыгрузкуИзФормы()
	...
	Если ЭтоКлиент И Не ПрямаяВыгрузка И Не Объект.ФлагОшибки Тогда
		Интерактивно = ПустаяСтрока(ИмяФайлаДанных);
		Если Интерактивно Тогда
			ИмяСохраняемогоФайла = ?(Объект.АрхивироватьФайл, НСтр("ru = 'Файл выгрузки.zip'"),НСтр("ru = 'Файл выгрузки.xml'"));
		Иначе
			ИмяСохраняемогоФайла = ИмяФайлаДанных;
		КонецЕсли;
		
		ПолучитьФайл(АдресФайлаДанныхВХранилище, ИмяСохраняемогоФайла, Интерактивно);
		ИмяФайлаДанных = ИмяСохраняемогоФайла;
	КонецЕсли;
...
Процедура ИзменитьДеревоПравилВыгрузки(СтрокиИсходногоДерева, СтрокиЗаменяемогоДерева)
	...
	КолонкаНастройкиПостроителя = СтрокиЗаменяемогоДерева.ВыгрузитьКолонку("НастройкиПостроителя");
...
Процедура ОткрытьФормуНастройкиОтладкиОбработчиков(ОбработчикиСобытийИзФайлаПравил)
	...
	ПараметрыОтладки = ОткрытьФорму(ИмяВызываемойФормы, ПараметрыФормы, Объект,,,,Обработчик, Режим);
...
Процедура ИзменитьРасположениеФайла()
	...
	ШаблонЗаголовкаПоляИмениФайла = "Имя файла %1 (%2)";
	Элементы.ФормаЗагрузитьПравила.Заголовок = СтрШаблон(ШаблонЗаголовкаПоляИмениФайла, "правил", ?(ЭтоКлиент, "клиент", "сервер"));
	Элементы.ИмяФайлаДанных.Заголовок = СтрШаблон(ШаблонЗаголовкаПоляИмениФайла, "данных", ?(ЭтоКлиент, "клиент", "сервер"));
...
&НаСервере
Процедура ЗаполнитьОтборКомпоновкиДанныхНаСервере(НастройкиПостроителя,ИмяОбъектаДляЗапроса,СтандартнаяОбработка)	 
	Компоновщик.Настройки.Отбор.Элементы.Очистить();
	Если НЕ ЗначениеЗаполнено(ИмяОбъектаДляЗапроса) Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	ИмяОбъектаДляЗапроса.Ссылка
	|ИЗ
	|	"+ИмяОбъектаДляЗапроса+" КАК ИмяОбъектаДляЗапроса";

	СхемаКомпоновкиДанных = Новый СхемаКомпоновкиДанных;
	//1. Определяем источники данных
	ИсточникДанных = СхемаКомпоновкиДанных.ИсточникиДанных.Добавить();
	ИсточникДанных.Имя = "ИсточникДанных1";
	ИсточникДанных.СтрокаСоединения = "";
	ИсточникДанных.ТипИсточникаДанных = "Local";
	//2. Определяем наборы данных
	НаборДанныхЗапрос = СхемаКомпоновкиДанных.НаборыДанных.Добавить(Тип("НаборДанныхЗапросСхемыКомпоновкиДанных"));
	НаборДанныхЗапрос.Имя = "НаборыДанных1";
	...
	Выбор = ГруппировкаКомпоновкиДанных.Выбор.Элементы.Добавить(Тип("АвтоВыбранноеПолеКомпоновкиДанных"));
	Выбор.Использование = Истина;
	
	URLСхемы = ПоместитьВоВременноеХранилище(СхемаКомпоновкиДанных,ЭтаФорма.УникальныйИдентификатор);
	...
	Для каждого Настройка из НастройкиПостроителя.Элементы Цикл
		СкопироватьНастройку(Компоновщик.Настройки.Отбор.Элементы,Настройка)
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура СкопироватьНастройку(НовыеЭлементы,Элемент)
	НоваяСтрока = НовыеЭлементы.Добавить(ТипЗнч(Элемент));
	ЗаполнитьЗначенияСвойств(НоваяСтрока,Элемент);
	Если ТипЗнч(Элемент) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда		
		Для каждого Настройка из Элемент.Элементы цикл
			СкопироватьНастройку(НоваяСтрока.Элементы,Настройка)	
		Конеццикла;
	КонецЕсли;
КОнецПроцедуры
...
Процедура ТаблицаПравилВыгрузкиПриАктивизацииСтроки(Элемент)
	...
	НастройкиПостроителя = ТекущиеДанные.НастройкиПостроителя;
	ЗаполнитьОтборКомпоновкиДанныхНаСервере(НастройкиПостроителя,ТекущиеДанные.ИмяОбъектаДляЗапроса,СтандартнаяОбработка);
	Элементы.КомпоновщикНастройкиОтбор.Доступность = СтандартнаяОбработка;
КонецПроцедуры
...
Процедура КомпоновщикНастройкиОтборПриИзменении(Элемент)
	...
	Если ТекущиеДанные = неопределено Тогда
		Возврат;
	КонецЕсли;
	ТекущиеДанные.НастройкиПостроителя = Компоновщик.Настройки.Отбор;
	ТекущиеДанные.ИспользоватьОтбор = (ТекущиеДанные.НастройкиПостроителя.Элементы.Количество() <> 0) ;
