...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокВыбора = Элементы.ОбщиеРедакторТема.СписокВыбора;
	СписокВыбора.Добавить(ИТК_РедакторКодаКлиентСервер.ТемаКонфигуратор(), НСтр("ru = 'Конфигуратор'; en = 'Configurator'"));
	СписокВыбора.Добавить(ИТК_РедакторКодаКлиентСервер.ТемаЕДТ(), НСтр("ru = 'EDT'; en = 'EDT'"));
	...
	ИТК_ИнтеграцияOneDebugger.УстановитьВидимостьНастройкиПодключения(ЭтотОбъект);
	...
	ИТК_РедакторКода.Инициализация(ЭтотОбъект, "РедакторПример", ДополнительныеПараметры);
...
Процедура ОбщиеРедакторНастройкиПриИзменении(Элемент)
	...
	Настройки.Вставить("Тема", ОбщиеРедакторТема);
...
&НаКлиенте
Процедура ПодключаемаяДобавитьКластер(Команда)
...
Процедура ОбновитьЭлементыФормы()
	...
	Элементы.ГруппаИнформацияДляФайловойБазы.Видимость = ИТК_ОбщийКлиентСервер.ИнформационнаяБазаФайловая();
...
Процедура ОбновитьЭлементыПодключенийККластерам(ПодключенияККластерам)
	...
	Для Каждого ЭлементПодключение Из ПодключенияККластерам Цикл 
		...
		Если НомерПодключения % КоличествоПодключенийВГруппе = 1 Тогда
			...
			Поля.Вставить("Заголовок", СтрШаблон(НСтр("ru = 'Подключения к кластера №%1';
														|en = 'Connections to cluster #%1'"), НомерГруппыПодключений));
		...
		Поля.Вставить("ОтображатьЗаголовок", Ложь);
		Поля.Вставить("Заголовок", СтрШаблон(НСтр("ru = 'Подключения к кластеру №%1';
													|en = 'Connections to cluster #%1'"), НомерПодключения));
		...
		ИТК_Форма.ДобавитьКоманду(ЭтотОбъект, ИмяКоманды, "ПодключаемыйИзменитьКластер",
									НСтр("ru = 'Изменить кластер';
										|en = 'Change cluster'"), ПараметрыКоманды);
		...
		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Подсказка", НСтр("ru = 'Удалить кластер';
													|en = 'Delete cluster'"));
		ИТК_Форма.ДобавитьКоманду(ЭтотОбъект, ИмяКоманды, "ПодключаемыйУдалитьКластер",
										НСтр("ru = 'Удалить';
											|en = 'Delete'"), ПараметрыКоманды);
