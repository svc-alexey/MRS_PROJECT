...
Функция ПолноеИмяСлужебногоПользователя(ИдентификаторПользователяИБ)
	...
	Если Не(ИТК_БСППовтИсп.РазделениеВключено()
			И ИТК_БСППовтИсп.СуществуетПодсистема("ТехнологияСервиса")) Тогда
		Возврат "";
	КонецЕсли;
	
	ПараметрыФункции = Новый Массив;
	ПараметрыФункции.Добавить(ИдентификаторПользователяИБ);
	Результат = ИТК_БСПКлиентСервер.ВыполнитьФункцию(
		"РаботаВМоделиСервиса.ПсевдонимПользователяИнформационнойБазы", ПараметрыФункции);
