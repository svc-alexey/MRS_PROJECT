...
Функция ЗагрузитьДанныеИзНастроек(Имя, Пользователь = Неопределено) Экспорт
	...
	Результат = ХранилищеСистемныхНастроек.Загрузить(КлючНастроек, Имя, , Пользователь);
...
Процедура ПереносНастроекПриПереходеУИРКToolkit() Экспорт
	...
	КлючНастроек = ИТК_КонсольРазработчикаДанныеКлиентСервер.КлючНастроек();
	ПереносНастроек("Консоль разработчика (УИР) (данные)", КлючНастроек);
...
Процедура СохранитьДанные(Имя, Данные) Экспорт
	...
	ДвоичныеДанныеMCR = ИТК_MCRФормат.ДвоичныеДанныеMCR(Данные);
	...
	ОписаниеНастроек.КлючОбъекта = Имя;
	ОписаниеНастроек.Представление = ТекущаяДатаСеанса();
	
	ХранилищеСистемныхНастроек.Сохранить(КлючНастроек, Имя, ЗаписываемыеДанные, ОписаниеНастроек);
	
КонецПроцедуры

Процедура СохранитьДанныеВФонеСлужебная(Параметры, АдресРезультата = Неопределено) Экспорт

	СохранитьДанные(Параметры.Имя, Параметры.Данные);
	ИТК_КонсольРазработчика.ДобавитьВПоследниеРабочиеДанные(Параметры.Имя, ИТК_Перечисления.ТипХраненияДанныхНастройка());
	
КонецПроцедуры

Функция СохранитьДанныеВФоне(Имя, Данные) Экспорт
	
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("Имя", Имя);
	ПараметрыПроцедуры.Вставить("Данные", Данные);
	
	ПараметрыВыполнения = ИТК_ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	
	ВозможноВыполнениеВФоне = ИТК_БСПКлиентСервер.ВозможноВыполнениеВФоне();
	ИТК_ДлительныеОперации.ВыполнитьВФоне("ИТК_КонсольРазработчика.СохранитьДанныеВФонеСлужебная", ПараметрыПроцедуры, ПараметрыВыполнения); 
	
	Возврат ВозможноВыполнениеВФоне;
...
Процедура УдалитьДанныеВНастройках(Имя) Экспорт
	...
	УдалитьИзПоследнихРабочихДанных(Имя, ИТК_Перечисления.ТипХраненияДанныхНастройка());
...
Функция ПоследниеРабочиеДанные() Экспорт
	...
	Для Каждого СтрокаТЗ Из Результат Цикл 
		...
		СтрокаТЗ.Тип = ИТК_НастройкиКлиентСерверПовтИсп.ПреобразованиеПеречисленияТипХраненияДанных(СтрокаТЗ.Тип, Истина);
...
Процедура СохранитьПоследниеРабочиеДанные(Значение) Экспорт
	...
	Для Каждого СтрокаТЗ Из Результат Цикл 
		...
		СтрокаТЗ.Тип = ИТК_НастройкиКлиентСерверПовтИсп.ПреобразованиеПеречисленияТипХраненияДанных(СтрокаТЗ.Тип);
...
Функция ПоследниеРабочиеДанныеВсе() Экспорт
	...
	ПоследниеРабочиеФайлы = ПоследниеРабочиеДанные.Скопировать(Новый Структура("Тип", ИТК_Перечисления.ТипХраненияДанныхФайл()), "Имя").ВыгрузитьКолонку("Имя");
	ПоследниеРабочиеНастройки = ПоследниеРабочиеДанные.Скопировать(Новый Структура("Тип", ИТК_Перечисления.ТипХраненияДанныхНастройка()), "Имя").ВыгрузитьКолонку("Имя");
...
Процедура АвтоматическоеСохранение(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	СохраняемыеДанные.Вставить("Данные", Новый ХранилищеЗначения(ИТК_MCRФормат.ДвоичныеДанныеMCR(Параметры.Данные), Новый СжатиеДанных(9)));
	СохраняемыеДанные.Вставить("Тип", Параметры.Тип);
	
	ИТК_КонсольРазработчика.ЗаписатьЗначениеПеременной("АвтоматическоеСохранениеДанных", СохраняемыеДанные);
...
Процедура ПодготовитьДанныеДляСравнения(Параметры, АдресРезультата) Экспорт
	...
	Результат.Вставить("Эталон", ПодготовитьРезультатДляСравнения(Параметры.Эталон, 
				ИТК_Строки.ТекстЭтапаПроцесса(НСтр("ru = 'Первый результат';
													|en = 'First result'"), Шаг, КоличествоШагов)));
	
	Результат.Вставить("Новый", ПодготовитьРезультатДляСравнения(Параметры.Новый, 
				ИТК_Строки.ТекстЭтапаПроцесса(НСтр("ru = 'Второй результат';
													|en = 'Second result'"), Шаг, КоличествоШагов)));
...
Функция ЗначенияВнешнихИсточников(Данные, СКД = Ложь) Экспорт
	...
	Для Каждого ВнешнийИсточник Из ИТК_КонсольРазработчикаДанныеКлиентСервер.ВнешниеИсточники(Данные) Цикл 
		
		ОписаниеЗначения = ВнешнийИсточник.Значение;
		Имя = ОписаниеЗначения.Имя;
		
		Если ОписаниеЗначения.Тип = ИТК_Перечисления.ТипЗначенияВнешнегоИсточникаВыражение() Тогда
			
			Комментарий = СтрШаблон(НСтр("ru = 'Внешний источник: %1';
										|en = 'External source: %1'"), Имя);
			Значение = ИТК_ОбщийВызовСервера.ВычислитьВыражениеНаВстроенномЯзыке(ОписаниеЗначения.Значение, Комментарий);
...
Функция ЗначенияТаблицМВТ(Данные, СКД = Ложь) Экспорт
	...
	Для Каждого ВнешнийИсточник Из ИТК_КонсольРазработчикаДанныеКлиентСервер.ТаблицыМВТ(Данные) Цикл 
...
Функция ПодготовитьРезультатДляСравнения(Данные, ГлавнаяОперация)
	...
	Параметры.Вставить("ПодготовкаСравнения", Истина);
	
	Если ИТК_КонсольРазработчикаДанныеКлиентСервер.ЭтоОдинИзТиповСтрокиЗапрос(Данные.Тип) Тогда
		
		Параметры.Вставить("Режим", ИТК_Перечисления.РежимВыполненияЗапросаВыполнение());
		Параметры.Вставить("ИгнорироватьЛимитСтрок", Истина);
		Возврат ИТК_Запрос.ПолучитьРезультат(Параметры);
		
	ИначеЕсли ИТК_КонсольРазработчикаДанныеКлиентСервер.ЭтоОдинИзТиповСтрокиСКД(Данные.Тип) Тогда
		
		Параметры.Вставить("Режим", ИТК_Перечисления.РежимВыполненияСКДВыполнение());
		Возврат ИТК_СКД.ПолучитьРезультат(Параметры);
