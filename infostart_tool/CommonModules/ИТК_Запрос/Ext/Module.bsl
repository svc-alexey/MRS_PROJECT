...
Функция ПолучитьРезультат(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	Если Не ОшибкаПриВыполнении Тогда
		...
		Если ВыводРезультата Тогда
			...
			ОформлятьОсобыеЗначенияВРезультате = Настройки["ОформлятьОсобыеЗначенияВРезультате"];
			...
			Для ИндексРезультата = 0 По РезультатыВыполнения.Результат.ВГраница() Цикл 
				...
				Области = ПодготовитьОбластиВыводаРезультата(РезультатВыполнения.Колонки, ОформлятьОсобыеЗначенияВРезультате, Настройки["ЧередованиеЦветовСтрокВРезультате"]);
				...
				Если ВывестиТаблицу Тогда
					...
					// Вывод строк таблицы
					Если ОформлятьОсобыеЗначенияВРезультате Тогда
					...
					Если АлгоритмОпределенияОптимальнойШириныКолонок = 0 Тогда // Ширина по Наименованию колонки (простой и быстрый алгоритм)
						...
						Для Каждого Колонка Из РезультатВыполнения.Колонки Цикл 
							ШиринаКолонок.Добавить(Макс(СтрДлина(Колонка.Имя) + 1 + ШиринаКартинки, Колонка.Ширина));
						КонецЦикла;
					...
					ИначеЕсли АлгоритмОпределенияОптимальнойШириныКолонок = 1 Тогда // Ширина по содержимому, анализируем первые 10 строк (более сложный алгоритм)
						...
						Для Каждого Колонка Из РезультатВыполнения.Колонки Цикл 
							ШиринаКолонок.Добавить(СтрДлина(Колонка.Имя) + 1 + ШиринаКартинки);
						КонецЦикла;
						...
						Для НомерКолонки = 1 По ШиринаКолонок.Количество() Цикл  
							...
							Для НомерСтроки = 1 По КоличествоАнализируемыхСтрок Цикл 
								...
								ШиринаКолонки = ДлинаТекстаПоля * КоэффициентШириныКолонок(ДлинаТекстаПоля, ОбластьСтрока);
								Если ОбластьСтрока.Картинка <> Неопределено Тогда
									ШиринаКолонки = ШиринаКолонки + ШиринаКартинки;
								КонецЕсли;
								
								ШиринаКолонок[ИндексКолонки] = Макс(ШиринаКолонки, ШиринаКолонок[ИндексКолонки]);
			...
			Если ЗначениеЗаполнено(АдресРезультата) Тогда
				...
				Если РезультатыВыполнения.КоличествоЗапросовСРезультатом = 1 Тогда
					...
					КоличествоВРезультате = СтрШаблон(НСтр("ru = 'Таблица, %1';
															|en = 'Table, %1'"), ПояснениеЗаголовкаЗапросаВРезультате(ВыведеноСтрок, КоличествоСтрок));
...
Функция ПодготовитьОбластиВыводаРезультата(Колонки, ОформлятьОсобыеЗначения, ЧередованиеЦветовСтрок)
	...
	Для Каждого Колонка Из Колонки Цикл 
		...
		ЗаполнениеОбластиЗначения(ОблЗначениеПоля1, Колонка, ОформлятьОсобыеЗначения);
		ЗаполнениеОбластиЗначения(ОблЗначениеПоля2, Колонка, ОформлятьОсобыеЗначения);
...
Функция ПояснениеЗаголовкаЗапросаВРезультате(ВыведеноСтрок, КоличествоСтрок, ВсеСтрокиВыведены = Истина)
	...
	Иначе
		Результат = СтрШаблон(НСтр("ru = '%1 из %2, не все выведены';
									|en = '%1 from %2, not all rows outputed'"), ВыведеноСтрок, КоличествоСтрок);
	КонецЕсли;
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл 
		...
		Для НомерСтроки = 1 По СтрокиЗапроса.ВысотаТаблицы Цикл 
			...
			Если МетодРасшифровкиТаблицЗначений = ИТК_Перечисления.РасшифровкаТаблицЗначенийОтключена() Тогда
				...
				Текст = НСтр("ru = '<Таблица значений>';
							|en = '<Value table>'");
				Расшифровывать = Ложь;
			...
			Иначе
				...
				Если КоличествоСтрок = 0 Тогда
					Текст = НСтр("ru = '<Пустая таблица значений>';
								|en = '<Empty value table>'");
				Иначе
					Гиперссылка = Истина;
					Текст = СтрШаблон(НСтр("ru = 'Таблица значений (%1)';
											|en = 'Value table (%1)'"), КоличествоСтрок);
				КонецЕсли;
			...
			Иначе
				...
				Расшифровка = Новый Структура("Сообщение", НСтр("ru = 'Расшифровка отключена в настройках';
																|en = 'Decryption disabled in settings'"));
...
Процедура ДополнительнаяОбработкаСтрок(СтрокиЗапроса, Колонки)
	...
	// Оформление NULL, 0, Неопределено, ПустаяСтрока, ПустаяДата и т.д. - значений без представлений, которые нужно как-то различать
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл 
			...
			Если ОсобоеЗначениеПредставление = Неопределено Тогда
				...
				Если Расшифровка = Неопределено Тогда
					...
					ОбластьСтрока.Текст = НСтр("ru = '<Неопределено>';
												|en = '<Undefined>'");
					ОбластьСтрока.ЦветТекста = ЦветТекстаПустыхЗначенийРезультата;
	...
	Для НомерКолонки = 1 По Колонки.Количество() Цикл 
		...
		Если НЕ Колонка.ТипЗначения.СодержитТип(Тип("Тип")) Тогда
			Продолжить
		КонецЕсли;
		...
		Для НомерСтроки = 1 По КоличествоСтрок Цикл 
			...
			ОбластьСтрока.Текст = ИТК_Типы.ПредставлениеТипа(Расшифровка);
			ОбластьСтрока.Расшифровка = Неопределено;
			ОбластьСтрока.Картинка = ИТК_Типы.Картинка(Расшифровка);
...
Функция КоэффициентШириныКолонок(Ширина, Область)
	...
	ШрифтЖирный = Область.Шрифт.Жирный;
...
Процедура ЗаполнениеОбластиЗначения(Область, Колонка, ОформлятьОсобыеЗначения)
	...
	Если Не (ОформлятьОсобыеЗначения
			 ИЛИ ТребуетсяРасшифровкаКолонкиРезультата(Колонка.ТипЗначения)) Тогда
		ПараметрРасшифровки = "";
	КонецЕсли;
