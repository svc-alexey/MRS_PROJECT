...
Функция Инициализация(Форма, Имя, ДополнительныеСвойства = Неопределено) Экспорт
	
	Если ДополнительныеСвойства = Неопределено Тогда
		ДополнительныеСвойства = Новый Структура;
	КонецЕсли;
	
	ТипЯзыка = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "ТипЯзыка", ИТК_Перечисления.ТипЯзыкаРедактораВстроенный());
	РежимКомпоновкиДанных = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "РежимКомпоновкиДанных", Ложь);
	ТолькоПросмотр = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "ТолькоПросмотр", Ложь);
	ВсегдаИспользоватьMonaco = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "ВсегдаИспользоватьMonaco", Ложь);
	
	УстановитьФокус = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "УстановитьФокус", Истина);
	ИспользованиеКоманд = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "ИспользованиеКоманд", Новый Структура);
	Текст = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "Текст");
	Подсказка = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "Подсказка", "");
	Ширина = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "Ширина", 50);
	Высота = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "Высота", 10);
	ФиксированныеНастройки = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеСвойства, "ФиксированныеНастройки", Новый Структура);
	...
	РедактируетсяКод = НЕ ТолькоПросмотр
							И ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораВстроенный();
	Если ИспользуетсяMonaco
			И РедактируетсяКод Тогда
			
		Если ЗначениеЗаполнено(Подсказка) Тогда
			Подсказка = Подсказка + Символы.ПС;
		КонецЕсли;
		Подсказка = Подсказка + ИТК_РедакторКодаКлиентСервер.ПодсказкаПоШаблонам(Язык);
	...
	ЦветФона = ИТК_ОбщийКлиентСервер.ЦветФонаЭлементРедактируется(НЕ ТолькоПросмотр);
	...
	ДополнительнаяИнформацияJSON = ИТК_ОбщийКлиентСервер.КоллекциюВJSONСтроку(ДополнительнаяИнформация);
	...
	ПрефиксРасширения = ИТК_ТуллкитКлиентСервер.Префикс();
	...
	ИнициализироватьЭлементы(Форма, ЭлементКод, ТипЯзыка, ИспользуетсяMonaco, ТолькоПросмотр, ИспользованиеКоманд);
...
Функция ОбработкаСобытияПолученияМетаданных(Объекты) Экспорт
	...
	Для Каждого ИмяОбъекта Из НаборОбъектов Цикл 
		...
		Если СтрНайти(ИмяОбъекта, ".") Тогда
			...
			ОписаниеОбъектаМетаданных = ИТК_РедакторКодаПовтИсп.ОписаниеОбъектаМетаданных(ИмяОбъекта);
		...
		Иначе
			...
			ИмяКоллекцииMonaco = ИТК_РедакторКодаКлиентСерверПовтИсп.ИмяКоллекцииМетаданныхПоТипу(ИмяОбъекта);
			Если ИмяКоллекцииMonaco = Неопределено Тогда
				Прервать;
			КонецЕсли;
			
			ОписаниеМетаданных = ИТК_РедакторКодаПовтИсп.ОписаниеСпискаОбъектовМетаданных(ИмяОбъекта);
			ОбластьОбновления = ИмяКоллекцииMonaco + ".items";
			
		КонецЕсли;
		
		ЭлементРезультата = Новый Структура;
		ЭлементРезультата.Вставить("ОписаниеМетаданных", ИТК_ОбщийКлиентСервер.КоллекциюВJSONСтроку(ОписаниеМетаданных));
	...
	Если ЗначениеЗаполнено(Свойства) Тогда
		...
		Для Каждого Свойство Из Свойства Цикл 
			...
			Если ТипЗнч(ЗначениеСвойства) = Тип("Структура") Тогда
				...
				Подсказка = ИТК_ОбщийКлиентСервер.Свойство(ЗначениеСвойства, "Подсказка");
				Если ЗначениеСвойства.Свойство("Тип") Тогда
					ОписаниеТиповЗначения = ЗначениеСвойства.Тип;
				Иначе
					ОписаниеТиповЗначения = Новый ОписаниеТипов;
				КонецЕсли;
				
				ТипЭлементов = ИТК_ОбщийКлиентСервер.Свойство(ЗначениеСвойства, "ТипЭлементов");
				
			Иначе
				
				Подсказка = Неопределено;
				ТипЭлементов = Неопределено;
			...
			Если ЗначениеЗаполнено(ТипЭлементов) Тогда
				ОписаниеСвойства.Вставить("item_ref", RefОписания(Кэш, ТипЭлементов));
			КонецЕсли;
...
Функция ИспользуетсяMonaco() Экспорт
	...
	Если Не ИТК_РедакторКодаКлиентСервер.ПоддерживаетсяПлатформойMonaco() Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Настройки = ИТК_Настройки.Загрузить();
	Результат = (Настройки["Редактор"] = ИТК_Перечисления.РедакторMonaco());
...
Функция ПользовательскиеОбъектыРезультат() Экспорт
	...
	ИмяПеременнойВозврата = ИТК_РедакторКодаКлиентСервер.ИмяПеременнойВозвратаРезультата();
	ДобавитьПользовательскийОбъект(Результат, ИмяПеременнойВозврата);
	
	ИмяПеременнойВозвратаEN = ИТК_РедакторКодаКлиентСервер.ИмяПеременнойВозвратаРезультата("en");
...
Функция ПользовательскиеОбъектыОбработкаРезультатов(Данные, АдресОбрабатываемыхДанных, Имя) Экспорт
	...
	ИдентификаторДополнительныеСвойства = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторДополнительныеСвойства();
	...
	ИдентификаторДополнительныеСвойстваEN = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторДополнительныеСвойства("en");
	...
	ИдентификаторПараметры = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторПараметры();
	ДобавитьПользовательскийОбъект(Результат, ИдентификаторПараметры, ОписаниеОбъектаПараметры);
	
	ИдентификаторПараметрыEN = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторПараметры("en");
	...
	Если КодПеред Тогда
		...
		Идентификатор = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторОбрабатываемыхДанных();
		ДобавитьПользовательскийОбъект(Результат, Идентификатор, ОписаниеОбъектаОбрабатываемыеДанные);
		
		ИдентификаторEN = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторОбрабатываемыхДанных("en");
	...
	Если ОсновнойКод Тогда
		...
		ИдентификаторСтроки = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторСтроки();
		ДобавитьПользовательскийОбъект(Результат, ИдентификаторСтроки, ОписаниеОбъектаСтрока);
		
		ИдентификаторСтрокиEN = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторСтроки("en");
...
Процедура ДобавитьПользовательскийОбъект(Объекты, Ключ, Описание = Неопределено) Экспорт
...
Функция ПримерКодаДляНастроек(Знач КодЯзыка) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(КодЯзыка) Тогда
		КодЯзыка = ИТК_ОбщийКлиентСерверПовтИсп.КодЯзыкаПрограммированияКонфигурации();
	КонецЕсли;
...
Функция RefТипа(Тип)
	...
	Иначе
		...
		ИмяОбъектаКоллекцииМетаданных = ИТК_Метаданные.ИмяОбъектаКоллекции(ОбъектМетаданных);
		RefКоллекции = ИТК_РедакторКодаПовтИсп.RefКоллекции(ИмяОбъектаКоллекцииМетаданных);
...
Процедура ИнициализироватьЭлементы(Форма, Элемент, ТипЯзыка, ИспользуетсяMonaco, ТолькоПросмотр, ИспользованиеКоманд)
	...
	РедактируетсяКод = (ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораВстроенный());
	РедактируетсяЗапрос = (ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораЗапросы());
	РедактируетсяXML = (ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораXML());
	РедактируетсяВыражениеСКД = (ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораВыраженияСКД());
	
	// Использование команд
	ИспользуетсяКонструктор = ИТК_ОбщийКлиентСервер.Свойство(ИспользованиеКоманд, "Конструктор", Истина);
	ИспользуетсяКонструкторВыделенного = ИТК_ОбщийКлиентСервер.Свойство(ИспользованиеКоманд, "КонструкторВыделенного", Истина);
	ИспользуетсяПреобразоватьВоВложенный = ИТК_ОбщийКлиентСервер.Свойство(ИспользованиеКоманд, "ПреобразоватьВоВложенный", Истина);
	ИспользуетсяВставкаПредопределенного = ИТК_ОбщийКлиентСервер.Свойство(ИспользованиеКоманд, "ВставкаПредопределенного", Истина);
	ИспользуетсяВставкаСсылкиНаОбъектБД = ИТК_ОбщийКлиентСервер.Свойство(ИспользованиеКоманд, "ВставкаСсылкиНаОбъектБД", Истина);
	ИспользуетсяУбратьПереносы = ИТК_ОбщийКлиентСервер.Свойство(ИспользованиеКоманд, "УбратьПереносы", Истина);
	ИспользуетсяПерейтиКСтроке = ИТК_ОбщийКлиентСервер.Свойство(ИспользованиеКоманд, "ПерейтиКСтроке", Истина);
	...
	КонтекстноеМенюГруппаРаботаСБуфероме = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюРаботаСБуфером, "Группа", , Элемент.КонтекстноеМеню);
	...
	Если ТекстРедактируется Тогда
		...
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ВырезатьВБуферОбмена);
		ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.X, , Истина));
		ЗаголовокКоманды = НСтр("ru = 'Вырезать';
								|en = 'Cut'");
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		
		Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
		ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюРаботаСБуфером + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаРаботаСБуфероме);
	...
	ЗаголовокКоманды = НСтр("ru = 'Копировать';
							|en = 'Copy'");
	ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если ТекстРедактируется Тогда
		...
		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ВставитьИзБуфераОбмена);
		ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.V, , Истина));
		ЗаголовокКоманды = НСтр("ru = 'Вставить';
								|en = 'Paste'");
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	ЗаголовокКоманды = НСтр("ru = 'Выделить все';
							|en = 'Select all'");
	ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если ТекстРедактируется
		...
		Если ИспользуетсяКонструктор Тогда
			...
			ЗаголовокКоманды = НСтр("ru = 'Конструктор...';
									|en = 'Constructor...'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
			
			Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
			ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМеню + ИмяКоманды, "Кнопка", Поля, Элемент.КонтекстноеМеню);
			...
			Если ИспользуетсяКонструкторВыделенного Тогда
				...
				ПараметрыКоманды = Новый Структура;
				ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.КонструкторЗапроса);
				ЗаголовокКоманды = НСтр("ru = 'Конструктор (выделенный текст)...';
										|en = 'Constructor (selected text)...'");
				ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		...
		КонтекстноеМенюГруппаРефакторинг = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюРефакторинг, "Группа", , Элемент.КонтекстноеМеню);
		...
		Если ИспользуетсяПреобразоватьВоВложенный Тогда
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ПреобразоватьВоВложенный);
			ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.S, , Истина, Истина));
			ЗаголовокКоманды = НСтр("ru = 'Преобразовать во вложенный...';
									|en = 'Convert to nested...'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
			
			Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
			ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюРефакторинг + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаРефакторинг);
	...
	Если ТекстРедактируется
		...
		Если ИспользуетсяВставкаПредопределенного Тогда
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ПредопределенныйЭлемент);
			ЗаголовокКоманды = НСтр("ru = 'Вставка предопределенного...';
									|en = 'Insert predefined...'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		...
		Если ИспользуетсяУбратьПереносы Тогда
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_УбратьПереносыСтрок);
			...
			ЗаголовокКоманды = ИТК_Перечисления.Представление(ИТК_Перечисления.АлгоритмОбработкиТекстаПереносЗапросаИзКонфигуратора());
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если ТекстРедактируется 
		...
		Если ИспользуетсяКонструктор Тогда
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.КонструкторЗапроса);
			ЗаголовокКоманды = НСтр("ru = 'Конструктор запроса...';
									|en = 'Constructor query...'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		...
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, НСтр("ru = 'Конструктор форматной строки...';
																		|en = 'Format string constructor...'"));
		
		Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
		ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМеню + ИмяКоманды, "Кнопка", Поля, Элемент.КонтекстноеМеню);
		
		ИмяКоманды = Имя + "_КонструкторСтрокиНаРазныхЯзыках";
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, НСтр("ru = 'Конструктор строк на разных языках...';
																		|en = 'String constructor in different languages ...'"));
		...
		Если ИспользуетсяВставкаСсылкиНаОбъектБД Тогда
			...
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, НСтр("ru = 'Вставка ссылки на объект БД...';
																			|en = 'Insert a reference to the database object...'"));
	...
	Если ИспользуетсяMonaco Тогда
		...
		КонтекстноеМенюГруппаНайтиЗаменить = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяГруппы, "Группа", , Элемент.КонтекстноеМеню);
		
		ИмяКоманды = Имя + "_Найти";
		
		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_Найти);
		ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.F, , Истина));
		ЗаголовокКоманды = НСтр("ru = 'Найти...';
								|en = 'Find...'");
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		
		Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
		ИТК_Форма.ДобавитьЭлемент(Форма, ИмяГруппы + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаНайтиЗаменить);
		...
		Если ТекстРедактируется Тогда
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_Заменить);
			ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.H, , Истина));
			ЗаголовокКоманды = НСтр("ru = 'Заменить...';
									|en = 'Replace...'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если РедактируетсяКод Или РедактируетсяЗапрос Тогда
		...
		ИмяКоманды = Имя + "_ПоделитьсяЧерезСервисКода";

		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_КопироватьВБуферОбмена);
		ЗаголовокКоманды = НСтр("ru = 'Копировать (поделиться) через paste1c.ru';
								|en = 'Copy (share) via paste1c.ru'");
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если РедактируетсяЗапрос Тогда

		ИмяКоманды = Имя + "_ТекстЗапросаКодНаВстроенномЯзыке";

		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_КопироватьВБуферОбмена);
		ЗаголовокКоманды = НСтр("ru = 'Копировать (код на встроенном языке)';
								|en = 'Copy (code in built-in language)'");
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если ИспользуетсяMonaco Тогда
		...
		Если ИспользуетсяПерейтиКСтроке Тогда
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ПерейтиКСтроке);
			ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.G, , Истина));
			ЗаголовокКоманды = НСтр("ru = 'Перейти к строке...';
									|en = 'Go to line ...'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		...
		Если РедактируетсяКод ИЛИ РедактируетсяЗапрос Тогда
			...
			ЗаголовокКоманды = НСтр("ru = 'Перейти к определению';
									|en = 'Go to definition'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		...
		Если ТекстРедактируется Тогда
			...
			Поля = Новый Структура("Заголовок", НСтр("ru = 'Изменения';
													|en = 'Changes'"));
			КонтекстноеМенюГруппаИзменения = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюИзменения, "Подменю", Поля, Элемент.КонтекстноеМеню);
			
			// Анализировать изменения
			ИмяКоманды = Имя + "_АнализироватьИзменения";
			
			ЗаголовокКоманды = НСтр("ru = 'Анализировать...';
									|en = 'Analyze...'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды);
			
			Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
			ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМеню + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаИзменения);
			...
			ЗаголовокКоманды = НСтр("ru = 'Зафиксировать';
									|en = 'Commit'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если ТекстРедактируется
		...
		Поля = Новый Структура("Заголовок", НСтр("ru = 'Комментирование';
												|en = 'Commenting'"));
		КонтекстноеМенюГруппаКомментирование = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюКомментирование, "Подменю", Поля, Элемент.КонтекстноеМеню);
		
		ИмяКоманды = Имя + "_ЗакомментироватьТекст";
		
		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ЗакомментироватьТекст);
		ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.NumDivide, , Истина));
		ЗаголовокКоманды = ИТК_Перечисления.Представление(ИТК_Перечисления.АлгоритмОбработкиТекстаЗакомментировать());
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		
		Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
		ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюКомментирование + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаКомментирование);
		
		ИмяКоманды = Имя + "_УбратьКомментарииВТексте";
		
		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_УбратьКомментарииВТексте);
		ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.NumDivide, , Истина, Истина));
		ЗаголовокКоманды = ИТК_Перечисления.Представление(ИТК_Перечисления.АлгоритмОбработкиТекстаУбратьКомментарии());
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
	...
	Если ИспользуетсяMonaco Тогда
		...
		Если РедактируетсяКод ИЛИ РедактируетсяЗапрос ИЛИ РедактируетсяXML Тогда
			...
			Поля = Новый Структура("Заголовок", НСтр("ru = 'Закладки';
													|en = 'Bookmarks'"));
			КонтекстноеМенюГруппаЗакладки = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюЗакладки, "Подменю", Поля, Элемент.КонтекстноеМеню);
			
			ИмяКоманды = Имя + "_УстановитьСнятьЗакладку";
			
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_УстановитьСнятьЗакладку);
			ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.F2, Истина));
			ЗаголовокКоманды = НСтр("ru = 'Установить/снять закладку';
									|en = 'Set/remove bookmark'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
			
			Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
			ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюЗакладки + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаЗакладки);
			
			ИмяКоманды = Имя + "_СледующаяЗакладка";
			
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_СледующаяЗакладка);
			ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.F2));
			ЗаголовокКоманды = НСтр("ru = 'Следующая закладка';
									|en = 'Next bookmark'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ПредыдущаяЗакладка);
			ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.F2, , , Истина));
			ЗаголовокКоманды = НСтр("ru = 'Предыдущая закладка';
									|en = 'Previous bookmark'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_УдалитьВсеЗакладки);
			ЗаголовокКоманды = НСтр("ru = 'Удалить все закладки';
		...
		Если ТекстРедактируется 
			...
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_Форматировать);
			ПараметрыКоманды.Вставить("СочетаниеКлавиш", Новый СочетаниеКлавиш(Клавиша.F, Истина, , Истина));
			ЗаголовокКоманды = НСтр("ru = 'Форматировать';
									|en = 'Format'");
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
			...
			КонтекстноеМенюГруппаПереносы = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяГруппыПереносы, "Группа", , Элемент.КонтекстноеМеню);
			
			ИмяКоманды = Имя + "_ДобавитьПереносыСтрок";
			
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_ДобавитьПереносыСтрок);
			ЗаголовокКоманды = ИТК_Перечисления.Представление(ИТК_Перечисления.АлгоритмОбработкиТекстаДобавитьПереносыСтрок());
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
			
			Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
			ИТК_Форма.ДобавитьЭлемент(Форма, ИмяГруппыПереносы + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаПереносы);
			
			ИмяКоманды = Имя + "_УбратьПереносыСтрок";
			
			ПараметрыКоманды = Новый Структура;
			ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_УбратьПереносыСтрок);
			ЗаголовокКоманды = ИТК_Перечисления.Представление(ИТК_Перечисления.АлгоритмОбработкиТекстаУбратьПереносыСтрок());
			ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		...
		КонтекстноеМенюГруппаНастройкаРедактора = ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюНастройкаРедактора, "Группа", , Элемент.КонтекстноеМеню);
		
		ИмяКоманды = Имя + "_НастройкаРедактора";
		
		ПараметрыКоманды = Новый Структура;
		ПараметрыКоманды.Вставить("Картинка", БиблиотекаКартинок.ИТК_Настройки);
		ЗаголовокКоманды = НСтр("ru = 'Настройка редактора...';
								|en = 'Configuring the Editor...'");
		ИТК_Форма.ДобавитьКоманду(Форма, ИмяКоманды, ИмяДействия, ЗаголовокКоманды, ПараметрыКоманды);
		
		Поля = Новый Структура("ИмяКоманды", ИмяКоманды);
		ИТК_Форма.ДобавитьЭлемент(Форма, ИмяКонтекстногоМенюНастройкаРедактора + ИмяКоманды, "Кнопка", Поля, КонтекстноеМенюГруппаНастройкаРедактора);
...
Функция ПредставлениеПоля(Поле)
	...
	Возврат СтрШаблон(Шаблон, ПредставлениеПоля, ИТК_Типы.ПредставлениеОписания(Поле.Тип));
