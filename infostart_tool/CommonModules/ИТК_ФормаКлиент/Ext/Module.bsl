...
Процедура ВводЗначенияСтрокиВФорме(Заголовок, Элемент, Значение, ОписаниеТипов, СтандартнаяОбработка, ОписаниеОповещенияЗавершение, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ПроверяемоеЗначение) = Тип("Строка") Тогда
		...
		ОткрытьМногострочныйТекст(Элемент, Элемент.ТекстРедактирования, , ДополнительныеПараметры);
...
Процедура АктивизацияЗначенияВФорме(Элемент, Значение, Знач ДоступныеТипы = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ПроверяемоеЗначение) = Тип("Дата") Тогда
		...
		Если Не ЗначениеСписок Тогда
			...
			Шаблон = НСтр("ru = 'Текущая дата, время (%1 ..)';
							|en = 'Current date, time (%1 ..)'");
...
Процедура АвтоПодборЗначенияВФорме(Текст, Элемент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	Если ИТК_Строки.ЭтоUID(Текст)
			...
			ИЛИ ИТК_Строки.ЭтоНавигационнаяСсылкаДанные(Текст) Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ИТК_Форма.АвтоПодборЗначения(Текст, Элемент.ДоступныеТипы, Элемент.ОграничениеТипа, ДанныеВыбора);
...
Процедура ОткрытьМногострочныйТекст(Владелец, Текст, Редактируется = Истина, ДополнительныеПараметры = Неопределено, СтандартнаяОбработка = Неопределено) Экспорт
...
Процедура ОткрытьРедактированиеТипаЗначения(Владелец, Заголовок, РежимРедактирования, Значение,
	...
	Параметры.Вставить("ТипыКод", ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ТипыКод", Ложь));
	Параметры.Вставить("ДоступныеОбъекты", ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ДоступныеОбъекты"));
	
	ВыборТипа = (РежимРедактирования = ИТК_Перечисления.РежимРедактированияТипаЗначенияВыборТипа())
					ИЛИ (РежимРедактирования = ИТК_Перечисления.РежимРедактированияТипаЗначенияВыборПроизвольногоТипа());
...
//
Процедура ОткрытьДиалогВыбораНастроекДанных(Владелец, Режим, Ключ, ТекущееИмя, ОписаниеОповещенияОЗакрытии) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Режим", Режим);
	ПараметрыФормы.Вставить("Ключ", Ключ);
	ПараметрыФормы.Вставить("ТекущееИмя", ТекущееИмя);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_ВыбораНастроекДанных", ПараметрыФормы, Владелец, , , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОткрытьТаблицуЗначений(Адрес, ФормаВладелец, Редактирование = Истина, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры) Тогда
		...
		Источник = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "Источник", НСтр("ru = 'Таблица';
																							|en = 'Table'"));
...
Процедура ОткрытьУниверсальнуюФормуСписка(Объект = Неопределено, Владелец = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	
	Параметры = Новый Структура;
	Параметры.Вставить("Объект", Объект);
	
	РежимВыбора = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "РежимВыбора", Ложь);
	Параметры.Вставить("РежимВыбора", РежимВыбора);
	
	ЗакрыватьПриВыборе = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ЗакрыватьПриВыборе", Ложь);
	Параметры.Вставить("ЗакрыватьПриВыборе", ЗакрыватьПриВыборе);
	
	Ключ = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "Ключ");
	Параметры.Вставить("Ключ", Ключ);
	
	Уникальность = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "Уникальность", Истина);
	...
	ОткрытьФорму("Обработка.ИТК_ФормаСпискаУниверсальная.Форма", Параметры, Владелец, Уникальность, , , ОписаниеОповещенияОЗакрытии, РежимОткрытия);
	
КонецПроцедуры

Процедура ОткрытьЖурналРегистрации(Параметры = Неопределено) Экспорт
	
	ОткрытьФорму("Обработка.ИТК_ЖурналРегистрации.Форма", Параметры);
...
Процедура ОткрытьФормуВыбора(Ссылка, Владелец, ЗакрыватьПриВыборе = Истина, Отбор = Неопределено) Экспорт
	...
	Имя = ИТК_Форма.ПолноеИмяФормыВыбораОбъекта(Ссылка, "ФормаВыбора");
...
Процедура ОткрытьКонструкторСхемыКомпоновкиДанных(Форма, XMLТекст) Экспорт
	...
	Если ТипКонструктора = ИТК_Перечисления.ТипКонструктораСКДТолстый() Тогда
		...
		#Если ТолстыйКлиентУправляемоеПриложение Тогда
			...
			Конструктор = Новый КонструкторСхемыКомпоновкиДанных(ИТК_СКД.СхемаКомпоновкиДанных(XMLТекст));
...
//
Процедура ЗадатьВопрос(ОписаниеОповещенияОЗавершении, Текст, Кнопки, КнопкаПоУмолчанию, Заголовок, Владелец, Идентификатор = Неопределено, ГруппаНастроек = Неопределено) Экспорт
	
	Если ЗначениеЗаполнено(ГруппаНастроек) Тогда
		БольшеНеПоказывать = НЕ ИТК_Настройки.Значение(ГруппаНастроек, Идентификатор);
	...
	Иначе 
		...
		ПараметрыФормы.Вставить("ГруппаНастроек", ГруппаНастроек);
...
Процедура ОбработкаТаблицаЗначенийРеквизитНачалоВыбора(Форма, Элемент, СтандартнаяОбработка) Экспорт
	...
	Если Элемент.ВыбиратьТип Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ИТК_ФормаКлиент.ОткрытьРедактированиеТипаЗначения(Элемент, НСтр("ru = 'Колонки';
																		|en = 'Column'"), ИТК_Перечисления.РежимРедактированияТипаЗначенияВыборТипа(),
...
Процедура РазвернутьДерево(Элемент, Коллекция, Все = Ложь) Экспорт
	...
	Если НЕ ЭлементВиден(Элемент) Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьПометки(Коллекция, Пометка = Истина, Поле = "Пометка") Экспорт
	...
	Для Каждого Строка Из ИсточникКоллекции(Коллекция) Цикл 
...
Процедура ОткрытьСравнениеТекстов(Форма, Текст1, Текст2, ТипЯзыка) Экспорт
	...
	Если Редактор = ИТК_Перечисления.РедакторОбычный() Тогда
		...
		#Если ТолстыйКлиентУправляемоеПриложение Тогда
			...
			ИТК_ФайлыКлиентСервер.ЗаписатьТекст(ИмяПервогоФайла, Текст1);
			ИТК_ФайлыКлиентСервер.ЗаписатьТекст(ИмяВторогоФайла, Текст2);
...
Процедура ПоискПоСпискуВыбора(Элемент, Поиск, ДанныеВыбора, СтандартнаяОбработка, Лимит = 30) Экспорт
...
Процедура ОткрытьВыборПравДоступа(Владелец, ВыбранныеПрава, ОписаниеОповещенияЗавершение, СтандартнаяОбработка = Неопределено) Экспорт
	...
	//ДоступныеПраваДоступа = ИТК_АнализПравДоступаПовтИсп.ВсеПраваДоступа();
	//Для Каждого ПравоДоступа Из ДоступныеПраваДоступа Цикл 
	//	
	//	Если ЗначениеЗаполнено(ВыбранныеПрава) Тогда
	//		Пометка = (ВыбранныеПрава.НайтиПоЗначению(ПравоДоступа.Значение) <> Неопределено);
	//	Иначе
	//		Пометка = Истина;
	//	КонецЕсли;
	//	
	//	ПравоДоступа.Пометка = Пометка;
	//	
	//КонецЦикла;
	
	ПараметрыФорма = Новый Структура;
	ПараметрыФорма.Вставить("ВыбранныеПрава", ВыбранныеПрава);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_ВыборПравДоступа", ПараметрыФорма, Владелец, , , , ОписаниеОповещенияЗавершение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОткрытьВыборРолей(Владелец, ВыбранныеРоли, ОписаниеОповещенияЗавершение, СтандартнаяОбработка = Неопределено) Экспорт
	...
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("Заголовок", НСтр("ru = 'Выбор ролей';
														|en = 'Role selection'"));
	
	ДоступныеРоли = ИТК_АнализПравДоступаПовтИсп.ВсеРолиСписок();
	Для Каждого ПравоДоступа Из ДоступныеРоли Цикл 
		
		ПравоДоступа.Пометка = НЕ ЗначениеЗаполнено(ВыбранныеРоли)
									ИЛИ (ВыбранныеРоли.НайтиПоЗначению(ПравоДоступа.Значение) <> Неопределено);
		
	КонецЦикла;
	
	ИТК_ФормаКлиент.ОткрытьСписокЗначений(Владелец, ИТК_Перечисления.ВариантФормыСписокЗначенийВыборНесколькихЗначениеСПредставлением(),
...
Процедура ОткрытьВыборПользователей(Владелец, ВыбранныеПользователи, ОписаниеОповещенияЗавершение, СтандартнаяОбработка = Неопределено) Экспорт
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Выбранные", ВыбранныеПользователи);

	ОткрытьФорму("ОбщаяФорма.ИТК_ВыборПользователейИБ", ПараметрыФормы, Владелец, , , , ОписаниеОповещенияЗавершение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыводОповещенийПодбора(ДобавленныеЗначения) Экспорт
	
	Если НЕ ЗначениеЗаполнено(ДобавленныеЗначения) Тогда
		Возврат
	КонецЕсли;
		
	Если ДобавленныеЗначения.Количество() = 1 Тогда
		ПредставлениеДобавленныхЗначений = ДобавленныеЗначения[0];
	Иначе
		ПредставлениеДобавленныхЗначений = ИТК_ФормаКлиентСервер.ПредставлениеСписка(ДобавленныеЗначения, 60);
	КонецЕсли;
	
	ТекстОповещения = СтрШаблон(НСтр("ru = 'В список добавлен(ы): %1';
									|en = 'Added to list: %1'"), ПредставлениеДобавленныхЗначений);
	ПоказатьОповещениеПользователя(ИТК_КонсольРазработчикаКлиентСервер.Представление(), , ТекстОповещения);
	
КонецПроцедуры

Процедура ОткрытьПоискКолонкиТаблицыЗначений(Владелец, Колонки, Имя, ОписаниеОповещенияОЗакрытии) Экспорт
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Имя", Имя);
	ПараметрыФормы.Вставить("Колонки", Колонки);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_НайтиКолонку", ПараметрыФормы, Владелец, , , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Функция КоличествоПомеченныхВКоллекции(Коллекция, ИмяПометки = "Пометка") Экспорт
	
	Результат = 0;
	Для Каждого Элемент Из ИсточникКоллекции(Коллекция) Цикл 
		
		Если Элемент[ИмяПометки] Тогда
			Результат = Результат + 1;
		КонецЕсли;
...
Процедура ОткрытьНовую(Форма) Экспорт
	
	ОткрытьФорму(Форма.ИмяФормы, , , Истина);
	
КонецПроцедуры

Процедура ДополнитьОтборСтрок(ЭлементКоллекция, Используется, ИмяПоля, ЗначениеПоля) Экспорт
	
	ОтборСтрок = Новый Структура(ЭлементКоллекция.ОтборСтрок);
	
	Если Используется Тогда
		ОтборСтрок.Вставить(ИмяПоля, ЗначениеПоля);
	Иначе
		
		Если ОтборСтрок.Свойство(ИмяПоля) Тогда 
	...
	Если ЗначениеЗаполнено(ОтборСтрок) Тогда
		ЭлементКоллекция.ОтборСтрок = Новый ФиксированнаяСтруктура(ОтборСтрок);
	Иначе
		ЭлементКоллекция.ОтборСтрок = Неопределено;
	КонецЕсли;

КонецПроцедуры

Процедура ОткрытьКонструкторНастроекКомпоновкиДанных(ФормаВладелец, АдресСхемыКомпоновкиДанных, АдресНастроек, ОписаниеОповещенияЗавершение) Экспорт
	
	ДополнительныеПараметры = Новый Структура;
	ДополнительныеПараметры.Вставить("ФормаВладелец", ФормаВладелец);
	ДополнительныеПараметры.Вставить("ОписаниеОповещения", ОписаниеОповещенияЗавершение);
	ДополнительныеПараметры.Вставить("АдресСхемыКомпоновкиДанных", АдресСхемыКомпоновкиДанных);
	ДополнительныеПараметры.Вставить("АдресНастроек", АдресНастроек);
	
	ОповещениеЗавершенияОткрытия = Новый ОписаниеОповещения("ЗавершениеОткрытияКонструктораНастроекКомпоновкиДанных", ЭтотОбъект, ДополнительныеПараметры);
	
	Если ИТК_СКД.ЗаполненыНастройкиКомпоновкиДанных(АдресСхемыКомпоновкиДанных, АдресНастроек) Тогда
		
		ТекстВопроса = НСтр("ru = 'Настройки введенные вручную будут потеряны'; en = 'Settings entered manually will be lost'");
		ПоказатьВопрос(ОповещениеЗавершенияОткрытия, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
		
	Иначе
		ВыполнитьОбработкуОповещения(ОповещениеЗавершенияОткрытия, КодВозвратаДиалога.ОК);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗавершениеОткрытияКонструктораНастроекКомпоновкиДанных(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("АдресСхемыКомпоновкиДанных", ДополнительныеПараметры.АдресСхемыКомпоновкиДанных);
	ПараметрыФормы.Вставить("АдресНастроек", ДополнительныеПараметры.АдресНастроек);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_КонструкторНастроекКомпоновкиДанных", ПараметрыФормы, ДополнительныеПараметры.ФормаВладелец, , , , ДополнительныеПараметры.ОписаниеОповещения);
	
КонецПроцедуры

Функция ЗначениеВозможноХранитьВПолеФормы(Значение) Экспорт
	
	ТипЗначения = ТипЗнч(Значение);
	ТипыКоторыеНельзяХранить = ИТК_ФормаКлиентПовтИсп.ТипыКоторыеНельзяХранитьВПолеФормы();
	Возврат (ТипыКоторыеНельзяХранить.Получить(ТипЗначения) = Неопределено);
	
КонецФункции

Процедура ОткрытьРедактированиеМассиваСтруктурыСоответствия(Владелец, НачальноеЗначение, ОписаниеОповещенияЗавершение) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НачальноеЗначение", НачальноеЗначение);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_РедактированиеМассиваСтруктурыСоответствия",
		ПараметрыФормы, Владелец, , , , ОписаниеОповещенияЗавершение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Процедура ОткрытьРедактированиеДвоичныхДанных(Владелец, НачальноеЗначение, ОписаниеОповещенияЗавершение) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НачальноеЗначение", НачальноеЗначение);
	
	ОткрытьФорму("ОбщаяФорма.ИТК_РедактированиеДвоичныхДанных",
		ПараметрыФормы, Владелец, , , , ОписаниеОповещенияЗавершение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

Процедура ОткрытьРедактированиеХранилищаЗначений(Значение, Владелец, ОписаниеОповещенияЗавершение) Экспорт
	
	Параметры = Новый Структура("НачальноеЗначение", Значение);
	ОткрытьФорму("ОбщаяФорма.ИТК_РедактированиеХранилищаЗначений", Параметры, Владелец, , , , ОписаниеОповещенияЗавершение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СвернутьВсеДополнительная(Элемент, Коллекция, ТребуемыйУровень = 1, Уровень = 1) Экспорт
	...
	Для Каждого Строка Из Коллекция.ПолучитьЭлементы() Цикл 
		...
		Если НЕ ЗначениеЗаполнено(Строка.ПолучитьЭлементы()) Тогда
			Продолжить;
		КонецЕсли;
...
Функция НайтиСтрокуДерева(Дерево, ИмяКолонки, ИскомоеЗначение) Экспорт
	...
	Для Каждого Строка Из Дерево.ПолучитьЭлементы() Цикл 
		...
		НайденнаяСтрока = НайтиСтрокуДерева(Строка, ИмяКолонки, ИскомоеЗначение);
...
Функция ИсточникКоллекции(Коллекция)
	
	Если ТипЗнч(Коллекция) = Тип("ДанныеФормыДерево") Тогда
		Результат = Коллекция.ПолучитьЭлементы();
	Иначе
		Результат = Коллекция;
	КонецЕсли;
