...
Функция СохранитьВФайл(Путь, Схема, Настройки = Неопределено, ВнешниеНаборыДанных = Неопределено, ИмяБезРасширения = Неопределено) Экспорт
	...
	Возврат ИТК_Отладка.СохранитьДанные(Схема, ИТК_Перечисления.ЭлементДанныхСхемаКомпоновкиДанных(), ИТК_Перечисления.ТипХраненияДанныхФайл(), ДополнительныеПараметры);
...
Функция Сохранить(Схема, Настройки = Неопределено, ВнешниеНаборыДанных = Неопределено, ИмяНастройки = Неопределено) Экспорт
	...
	Возврат ИТК_Отладка.СохранитьДанные(Схема, ИТК_Перечисления.ЭлементДанныхСхемаКомпоновкиДанных(), ИТК_Перечисления.ТипХраненияДанныхНастройка(), ДополнительныеПараметры);
...
//	Режим - Перечисление.ИТК_РежимВыполненияСхемыКомпоновкиДанных - Режим выполнения схемы компоновки данных
...
Функция ПолучитьРезультат(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	АнализИсполняемыхЗапросов = (Режим = ИТК_Перечисления.РежимВыполненияСКДПолучениеИсполняемыхЗапросов());
	...
	Попытка 
		...
		ИТК_ДлительныеОперации.СообщитьПрогресс(ИТК_Строки.ТекстЭтапаПроцесса(НСтр("ru = 'Получение схемы';
																					|en = 'Getting the schema'"), Шаг, КоличествоШагов, ГлавнаяОперация));
		СхемаКомпоновкиДанных = СхемаКомпоновкиДанных(Данные.XMLСхемаКомпоновкиДанных);
		
		ИТК_ДлительныеОперации.СообщитьПрогресс(ИТК_Строки.ТекстЭтапаПроцесса(НСтр("ru = 'Инициализация внешних источников';
																					|en = 'Initialization of external sources'"), Шаг, КоличествоШагов, ГлавнаяОперация));
		ВнешниеИсточники = ИТК_КонсольРазработчика.ЗначенияВнешнихИсточников(Данные, Истина);
		
		ТаблицыМВТ = ИТК_КонсольРазработчика.ЗначенияТаблицМВТ(Данные, Истина);
		МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
		ИТК_Запрос.ЗагрузкаДанныхМВТИзВнешнихИсточников(ТаблицыМВТ, МенеджерВременныхТаблиц);
	
		// Начало замера времени выполнения --------------------------------------------------------------------------------- {
		НачалоЗамера = ТекущаяУниверсальнаяДатаВМиллисекундах();
		
		ИТК_ДлительныеОперации.СообщитьПрогресс(ИТК_Строки.ТекстЭтапаПроцесса(НСтр("ru = 'Компоновка макета';
																					|en = 'Layout layout'"), Шаг, КоличествоШагов, ГлавнаяОперация));
		...
		ИТК_ДлительныеОперации.СообщитьПрогресс(ИТК_Строки.ТекстЭтапаПроцесса(НСтр("ru = 'Выполнение';
																					|en = 'Executing'"), Шаг, КоличествоШагов, ГлавнаяОперация));
		...
		Если ВыводРезультата Тогда
			...
			ИТК_ДлительныеОперации.СообщитьПрогресс(ИТК_Строки.ТекстЭтапаПроцесса(НСтр("ru = 'Вывод результата';
																						|en = 'The output'"), Шаг, КоличествоШагов, ГлавнаяОперация));
			...
			Если ИТК_ОбщийКлиентСервер.ПоддерживаетсяПлатформой("8.3.17") Тогда 
				ПроцессорКомпоновкиДанных.Инициализировать(МакетКомпоновкиДанных, ВнешниеИсточники, ДанныеРасшифровки, ИспользованиеВнешнихФункций, , МенеджерВременныхТаблиц);
			Иначе
...
//
Функция XMLТекстИзАдреса(Адрес) Экспорт
	
	Объект = ПолучитьИзВременногоХранилища(Адрес);
	Возврат XMLТекст(Объект);
...
Функция ДоступныеПоляВыбора(ОписаниеТипов) Экспорт
	...
	Для Каждого Тип Из ОписаниеТипов.Типы() Цикл
		...
		Для Каждого ОписаниеРеквизита Из ОбъектМетаданных.СтандартныеРеквизиты Цикл 
			...
			Если ИмяРеквизита = ИТК_МетаданныеКлиентСерверПовтИсп.СтандартноеИмяПоляОбъекта("Ссылка") Тогда
				Продолжить;
			КонецЕсли;
...
Функция ТипНабораДанныхСхемыКомпоновкиДанных(Значение) Экспорт
	...
	Если Значение = ИТК_Перечисления.СКДНаборДанныхЗапрос() Тогда
		
		Результат = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных");
		
	ИначеЕсли Значение = ИТК_Перечисления.СКДНаборДанныхОбъект() Тогда
...
Функция ПеречислениеТипНабораДанных(Тип) Экспорт
	...
	Если Тип = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
		...
		Результат = ИТК_Перечисления.СКДНаборДанныхЗапрос();
		
	ИначеЕсли Тип = Тип("НаборДанныхОбъединениеСхемыКомпоновкиДанных") Тогда
		
		Результат = ИТК_Перечисления.СКДНаборДанныхОбъединение();
		
	Иначе
		
		Результат = ИТК_Перечисления.СКДНаборДанныхОбъект();
...
Функция ТипПоляНабораДанныхСхемыКомпоновкиДанных(Значение) Экспорт
	...
	Если Значение = ИТК_Перечисления.СКДВидыПолейНаборовДанныхПоле() Тогда
		
		Результат = Тип("ПолеНабораДанныхСхемыКомпоновкиДанных");
		
	ИначеЕсли Значение = ИТК_Перечисления.СКДВидыПолейНаборовДанныхНабор() Тогда
...
Функция ПеречислениеТипПоляНабораДанных(Тип) Экспорт
	...
	Если Тип = Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") Тогда
		...
		Результат = ИТК_Перечисления.СКДВидыПолейНаборовДанныхПоле();
		
	ИначеЕсли Тип = Тип("ВложенныйНаборДанныхСхемыКомпоновкиДанных") Тогда
		
		Результат = ИТК_Перечисления.СКДВидыПолейНаборовДанныхНабор();
		
	Иначе
		
		Результат = ИТК_Перечисления.СКДВидыПолейНаборовДанныхГруппа();
...
Функция ПеречислениеВидМакета(ОписаниеМакета, Заголовки) Экспорт
	...
	Если ТипМакета = Тип("МакетПоляСхемыКомпоновкиДанных") Тогда
		...
		Результат = ИТК_Перечисления.СКДМакетПоля();
		
	ИначеЕсли ТипМакета = Тип("МакетГруппировкиСхемыКомпоновкиДанных") Тогда
		
		Если Заголовки Тогда
			Результат = ИТК_Перечисления.СКДМакетЗаголовкаГруппировки();
		Иначе
			Результат = ИТК_Перечисления.СКДМакетГруппировки();
		КонецЕсли;
		
	ИначеЕсли ТипМакета = Тип("МакетПолейИтогаСхемыКомпоновкиДанных") Тогда
		
		Результат = ИТК_Перечисления.СКДМакетРесурсов();
...
Функция ПредставлениеОписанияРолиПоляНабораДанных(Роль) Экспорт 
	...
	НомерПериода = ИТК_ОбщийКлиентСервер.Свойство(Роль, "НомерПериода", 0);
	Измерение = ИТК_ОбщийКлиентСервер.Свойство(Роль, "Измерение", Ложь);
	Счет = ИТК_ОбщийКлиентСервер.Свойство(Роль, "Счет", Ложь);
	Остаток = ИТК_ОбщийКлиентСервер.Свойство(Роль, "Остаток", Ложь);
	
	Если ЗначениеЗаполнено(НомерПериода) Тогда
		
		Результат.Добавить(НСтр("ru = 'Период';
								|en = 'Period'"));
		Результат.Добавить(НомерПериода);
		
		ТипПериода = ИТК_ОбщийКлиентСервер.Свойство(Роль, "ТипПериода", "Основной");
		Если ТипПериода = "Дополнительный" Тогда
			Результат.Добавить(НСтр("ru = 'Доп';
									|en = 'Add'"));
		КонецЕсли;
		
	ИначеЕсли Измерение Тогда
		
		Результат.Добавить(НСтр("ru = 'Измерение';
								|en = 'Dimension'"));
	...
	ИначеЕсли Счет Тогда
		...
		Результат.Добавить(НСтр("ru = 'Счет';
								|en = 'Account'"));
	...
	ИначеЕсли Остаток Тогда
		...
		Если ТипОстатка = "НачальныйОстаток" Тогда
			...
			Результат.Добавить(НСтр("ru = 'НачОст';
									|en = 'InBal'"));
			
		ИначеЕсли ТипОстатка = "КонечныйОстаток" Тогда
			
			Результат.Добавить(НСтр("ru = 'КонОст';
									|en = 'EndBal'"));
		...
		Если ТипБухгалтерскогоОстатка = "Дебет" Тогда
			...
			Результат.Добавить(НСтр("ru = 'Дебет';
									|en = 'Debit'"));
			
		ИначеЕсли ТипБухгалтерскогоОстатка = "Кредит" Тогда
			
			Результат.Добавить(НСтр("ru = 'Кредит';
									|en = 'Credit'"));
	...
	Обязательное = ИТК_ОбщийКлиентСервер.Свойство(Роль, "Обязательное", Ложь);
	Если Обязательное Тогда
		Результат.Добавить(НСтр("ru = 'Обязательное';
								|en = 'Mandatory'"));
	КонецЕсли;
	
	ИгнорироватьЗначенияNULL = ИТК_ОбщийКлиентСервер.Свойство(Роль, "ИгнорироватьЗначенияNULL", Ложь);
	Если ИгнорироватьЗначенияNULL Тогда
		Результат.Добавить(НСтр("ru = 'NULL';
								|en = 'NULL'"));
	КонецЕсли;
...
Функция ОписаниеРолиПоляНабораДанных(Роль = Неопределено) Экспорт
	...
	Результат = ИТК_СКДКлиентСервер.ПустоеОписаниеРолиПоляНабораДанных();
...
Процедура ЗаполнитьРольПолеНабораДанныхПоОписанию(Роль, Описание) Экспорт
	...
	ОписаниеРоли = ИТК_СКДКлиентСервер.ПустоеОписаниеРолиПоляНабораДанных();
	ЗаполнитьЗначенияСвойств(ОписаниеРоли, Описание);
	
	ТипПериода = ИТК_ОбщийКлиентСервер.Свойство(Описание, "ТипПериода", "Основной");
	ОписаниеРоли.ТипПериода = ТипПериодаКомпоновкиДанных[ТипПериода];
	
	ТипОстатка = ИТК_ОбщийКлиентСервер.Свойство(Описание, "ТипОстатка", "Нет");
	ОписаниеРоли.ТипОстатка = ТипОстаткаКомпоновкиДанных[ТипОстатка];
	
	ТипБухгалтерскогоОстатка = ИТК_ОбщийКлиентСервер.Свойство(Описание, "ТипБухгалтерскогоОстатка", "Нет");
...
Функция ИспользуемыеПараметры(СхемаКомпоновкиДанных) Экспорт
	...
	Для Каждого ПараметрДанных Из КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы Цикл 
		...
		Результат.Вставить(ИмяПараметра, ИТК_ТипыКлиентСервер.ОписаниеТиповПоТипу(ТипЗначения));
...
Функция ЛокализированнаяСтрокаНСтрИзXDTO(Объект, ИмяПоля) Экспорт
	...
	Если ТипЗнч(Представление) = Тип("Строка") Тогда
		...
		КодТекущегоЯзыка = ИТК_ОбщийКлиентСервер.КодТекущегоЯзыка();
	...
	Возврат ИТК_Строки.ЛокализированнаяСтрокаНСтрИзЗначения(Результат);
...
Процедура ЛокализированнаяСтрокаЗаписатьНСтрВСхемуXDTO(СхемаXDTO, ИмяПоля, Значение) Экспорт
	...
	ЛокализированнаяСтрокаЗначение = ИТК_Строки.ЛокализированнаяСтрокаЗначениеИзНСтр(Значение);
...
Функция ЗаполненыНастройкиКомпоновкиДанных(АдресСхемыКомпоновкиДанных, АдресНастроек) Экспорт

	КомпоновщикНастроек = Новый КомпоновщикНастроекКомпоновкиДанных;
	КомпоновщикНастроек.Инициализировать(Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресСхемыКомпоновкиДанных));
	
	НастройкиВарианта = ПолучитьИзВременногоХранилища(АдресНастроек);
	КомпоновщикНастроек.ЗагрузитьНастройки(НастройкиВарианта);
	
	Возврат ЗначениеЗаполнено(КомпоновщикНастроек.Настройки.Структура);
...
Функция ИсполняемыеЗапросы(СхемаКомпоновкиДанных, МакетКомпоновкиДанных)
	...
	НаборыДанных.Колонки.Добавить("Имя", ИТК_ТипыКлиентСервер.ОписаниеСтрока(200));
	НаборыДанных.Колонки.Добавить("Тип", ИТК_ТипыКлиентСервер.ОписаниеСтрока());
	НаборыДанных.Колонки.Добавить("Текст", ИТК_ТипыКлиентСервер.ОписаниеСтрока());
	НаборыДанных.Колонки.Добавить("ИсходныйТекст", ИТК_ТипыКлиентСервер.ОписаниеСтрока());
...
Функция ДанныеСвязейНаборовДанных(Связи)
	...
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("Источник", ИТК_ТипыКлиентСервер.ОписаниеСинонимМетаданных());
	Результат.Колонки.Добавить("Приемник", ИТК_ТипыКлиентСервер.ОписаниеСинонимМетаданных());
	Результат.Колонки.Добавить("ВыражениеИсточник", ИТК_ТипыКлиентСервер.ОписаниеСтрока());
	Результат.Колонки.Добавить("ВыражениеПриемник", ИТК_ТипыКлиентСервер.ОписаниеСтрока());
	Результат.Колонки.Добавить("Тип", ИТК_ТипыКлиентСервер.ОписаниеСтрока());
	Результат.Колонки.Добавить("Условие", ИТК_ТипыКлиентСервер.ОписаниеСтрока());
...
Процедура ДобавитьДоступноеПолеВыбора(ТаблицаПолей, Имя, Значение, Порядок = 1)
	...
	Иначе
		...
		Строка.Значение = ИТК_ТипыКлиентСервер.СложитьОписания(Строка.Значение, Значение);
...
Процедура ИспользуемыеПараметрыНаборовДанных(Источники, Результат)
	...
	Для Каждого НаборДанных Из Источники Цикл 
		...
		Если ТипНабораДанных = Тип("НаборДанныхЗапросСхемыКомпоновкиДанных") Тогда
			...
			ПараметрыЗапроса = ИТК_Запрос.НайтиПараметры(ТекстЗапроса);
