...
Функция ERFИзВременногоХранилища(АдресХранилища, УникальныйИдентификатор) Экспорт
	...
	ТекстСхемыКомпоновкиДанных = ИТК_Конфигуратор.СхемаКомпоновкиДанныхИзВнешнегоОтчета(ПолноеИмяФайлаОтчета);
	...
	НовыйЭлемент.Вставить("Name", НСтр("ru = 'Схема компоновки данных';
...
Функция ПрочитатьДанныеDCR(СтрокаДерева)
	...
	Для Каждого Строка Из СтрокаДерева.Строки Цикл 
		...
		Если Строка.ТипСтроки = 0 Тогда
			ТипСтроки = ИТК_Перечисления.ЭлементДанныхСхемаКомпоновкиДанных();
		ИначеЕсли Строка.ТипСтроки = 1 Тогда
			ТипСтроки = ИТК_Перечисления.ЭлементДанныхВариантОтчетаСКД();
		ИначеЕсли Строка.ТипСтроки = 2 Тогда
			ТипСтроки = ИТК_Перечисления.ЭлементДанныхПользовательскаяНастройкаСКД();
		ИначеЕсли Строка.ТипСтроки = 3 Тогда
			ТипСтроки = ИТК_Перечисления.ЭлементДанныхГруппа();
		КонецЕсли;
		
		ДанныеНовогоЭлемента.Вставить("Type", ИТК_MCRФормат.ЗначениеTypeВXML(ТипСтроки));
...
Функция Q1CЧтениеЗначенияПараметраЗапроса(СтрокаДерева)
	...
	ИначеЕсли ТипПараметра = "МоментВремени" Тогда
		...
		ЗначениеПараметра = ИТК_ОбщийВызовСервера.НовыйМоментВремени(Дата, Ссылка);
...
Функция Q1CЧтениеЗначенияТаблицыЗначений(СтрокаДерева)
	...
	Для Каждого СтрокаItem Из СтрокаДерева.Строки Цикл 
		...
		ДлинаТипаКолонки = ИТК_ОбщийКлиентСервер.Свойство(ДанныеЯчейки, "Length", 0);
...
Функция ПрочитатьДанныеОтладкиИзСтрокиXML(Данные)
	...
	ДанныеНовогоЭлемента = Новый Структура;
	ДанныеНовогоЭлемента.Вставить("Name", НСтр("ru = 'Запрос';
												|en = 'Query'"));
	ДанныеНовогоЭлемента.Вставить("Type", "Query");
	
	// Текст запроса
	Текст = ИТК_ОбщийКлиентСервер.Свойство(Данные, "Текст", "");
	ДанныеНовогоЭлемента.Вставить("Text", Текст);
	
	// Значения параметров запроса
	ЗначенияПараметровЗапроса = Новый Соответствие;
	Параметры = ИТК_ОбщийКлиентСервер.Свойство(Данные, "Параметры");
