...
Процедура Инициализация(Форма, Имя) Экспорт
	...
	Если ИТК_РедакторКодаКлиентСервер.ЭтоРедакторКодаMonaco(Элемент) Тогда
...
Процедура ПолучениеРабочегоКаталогаДанныхПользователяЗакончено(КаталогДанныхПользователя, ДополнительныеПараметры) Экспорт
	...
	Если ИТК_ОбщийКлиент.ЭтоWindowsКлиент() Тогда
		ШаблонИмениКаталога = "%1\ITK_MonacoWin%2\";
	Иначе
		ШаблонИмениКаталога = "%1\ITK_Monaco%2\";
	КонецЕсли;
	ПолноеИмяКаталога = СтрШаблон(ШаблонИмениКаталога, КаталогДанныхПользователя, ИТК_ТуллкитКлиентСервер.Версия());
...
Процедура ДополнительнаяИнициализация(Форма, Элемент, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Текст = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "Текст", "");
	ТипЯзыка = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ТипЯзыка", ИТК_РедакторКодаКлиентСервер.ТипЯзыка(Элемент));
	ПользовательскиеОбъекты = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ПользовательскиеОбъекты");
	УстановитьФокус = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "УстановитьФокус", Истина);
	ЗагружатьОбщиеМодули = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ЗагружатьОбщиеМодули", Истина);
	ОписаниеПользовательскихОбщихМодулейJSON = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ОписаниеПользовательскихОбщихМодулейJSON");
	
	ТолькоПросмотр = Элемент.ТолькоПросмотр;
	РежимЗапрос = (ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораЗапросы());
	РежимКод = (ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораВстроенный());
	...
	defaultView.setOption("generateEscapeEvent", Истина);
	...
	НастройкиРедактора = ИТК_РедакторКодаВызовСервера.ЗагрузитьНастройкиРедактора();
	...
	ПрименитьНастройкиРедактораКОбъекту(Элемент, НастройкиРедактора);

	Если РежимКод Тогда

		Если ЗагружатьОбщиеМодули Тогда
			ОписаниеОбщихМодулей = ИТК_РедакторКодаПовтИсп.ОписаниеОбщихМодулейJSON();
		КонецЕсли;
	...
	Если НЕ ТолькоПросмотр Тогда
		...
		Если РежимКод Тогда
			...
			// Загружаем шаблоны
			СтрокаJSON = ИТК_РедакторКодаКлиентСервер.ШаблоныJSONСтрока();
		...
		Если ЗначениеЗаполнено(ПользовательскиеОбъекты) Тогда
			...
			СтрокаJSON = ИТК_РедакторКодаКлиентСервер.ПользовательскиеОбъектыВJSONСтроку(ПользовательскиеОбъекты);
	...
	ИТК_ИнтеграцияOneDebuggerКлиент.ДополнительнаяИнициализация(Форма, Элемент.Имя);
...
Процедура УстановитьТекст(Форма, Элемент, Текст = Неопределено) Экспорт
	...
	Если ИТК_РедакторКодаКлиентСервер.ЭтоРедакторКодаMonaco(Элемент) Тогда
		...
		// Странное поведение если установить Текст = Неопределено редактор перестает отправлять сообщения
		Если Текст = Неопределено Тогда
			Текст = "";
		КонецЕсли;
		
		defaultView.setContent(Текст);
...
Процедура УстановитьДоступностьРедактирования(Форма, Элемент, ДоступноРедактирование) Экспорт
	...
	Иначе
		...
		Элемент.ЦветФона = ИТК_ОбщийКлиентСервер.ЦветФонаЭлементРедактируется(ДоступноРедактирование);
...
Функция ТребуетсяИнициализация(Элемент) Экспорт
	...
	Возврат НЕ Инициализирован(Элемент)
				ИЛИ (ИТК_РедакторКодаКлиентСервер.ЭтоРедакторКодаMonaco(Элемент)
						И НЕ ИТК_ФормаКлиент.ЭлементВиден(Элемент));
...
Процедура ПодключаемыйОбработчикКоманд(Форма, Имя, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	// В основном расширении имена Код_УстановитьЗакладку в КД3 - КД3ИТК_ТекстЗапроса_УстановитьЗакладку
	Индекс = 1;
	ИмяПоля = ИТК_Строки.РазборПрочитатьДоСимвола(Имя, "_", Индекс);
	Если СтрЧислоВхождений(Имя, "_") = 2 Тогда
		ИмяПоля = ИмяПоля + "_" + ИТК_Строки.РазборПрочитатьДоСимвола(Имя, "_", Индекс + 1);
	КонецЕсли;
	...
	Если СтрНайти(Имя, "ЗакомментироватьТекст") Тогда
		...
		Иначе
			...
			ОбработкаВыделенногоТекстаПоАлгоритму(Элемент, Текст, ИТК_Перечисления.АлгоритмОбработкиТекстаЗакомментировать());
	...
	ИначеЕсли СтрНайти(Имя, "УбратьКомментарииВТексте") Тогда
		...
		Иначе
			...
			ОбработкаВыделенногоТекстаПоАлгоритму(Элемент, Текст, ИТК_Перечисления.АлгоритмОбработкиТекстаУбратьКомментарии());
	...
	ИначеЕсли СтрНайти(Имя, "ПоделитьсяЧерезСервисКода") Тогда
		
		ПоделитьсяЧерезСервисКода(Текст, ИТК_РедакторКодаКлиентСервер.ТипЯзыка(Элемент));
		
	ИначеЕсли СтрНайти(Имя, "ТекстЗапросаКодНаВстроенномЯзыке") Тогда
		
		ТекстЗапросаКодНаВстроенномЯзыке(Текст);
	...
	ИначеЕсли СтрНайти(Имя, "АнализироватьИзменения") Тогда
		...
		ТипЯзыка = ИТК_РедакторКодаКлиентСервер.ТипЯзыка(Элемент);
	...
	ИначеЕсли СтрНайти(Имя, "Вырезать") Тогда
		...
		ИТК_БуферОбменаКлиент.Копировать(ВыделенныйТекст);
	...
	ИначеЕсли СтрНайти(Имя, "Вставить") Тогда
		...
		ТекстВБуфере = ИТК_БуферОбменаКлиент.Текст();
	...
	ИначеЕсли СтрНайти(Имя, "КонструкторВыделенногоЗапроса") Тогда
		...
		РежимКомпоновкиДанных = ИТК_РедакторКодаКлиентСервер.РежимКомпоновкиДанных(Элемент);
		ИТК_ЗапросКлиент.ОткрытьКонструктор(ВыделенныйТекст, ОповещениеОЗавершении, РежимКомпоновкиДанных);
	...
	ИначеЕсли СтрНайти(Имя, "КонструкторФорматнойСтроки")
		...
		ОткрытьКонструкторВыраженияВКавычках(Форма, Элемент, Имя, Текст);

	ИначеЕсли СтрНайти(Имя, "КонструкторЗапроса") Тогда
		
		РежимКомпоновкиДанных = ИТК_РедакторКодаКлиентСервер.РежимКомпоновкиДанных(Элемент);
		ИТК_ЗапросКлиент.ОткрытьКонструктор(Текст, ОповещениеОЗавершении, РежимКомпоновкиДанных);
		
	ИначеЕсли СтрНайти(Имя, "ПереносЗапросаИзКонфигуратора") Тогда
		
		УстановитьВыделениеВесьТекст(Элемент, Текст);
		ОбработкаВыделенногоТекстаПоАлгоритму(Элемент, Текст, ИТК_Перечисления.АлгоритмОбработкиТекстаПереносЗапросаИзКонфигуратора());
		
	ИначеЕсли СтрНайти(Имя, "ПреобразоватьВоВложенный") Тогда
		
		ЗаголовокПредупреждения = ИТК_КонсольРазработчикаКлиентСервер.Представление();
		ИТК_ЗапросКлиент.ОткрытьПреобразоватьВоВложенный(Форма, Элемент, ЗаголовокПредупреждения, ОповещениеОЗавершении);
	...
	ИначеЕсли СтрНайти(Имя, "ВставкаПредопределенного") Тогда
		...
		ИТК_ЗапросКлиент.ОткрытьВставкуПредопределенного(Форма, Элемент);
		
	ИначеЕсли СтрНайти(Имя, "НастройкаРедактора") Тогда
		
		ИТК_ОбщийКлиент.ОткрытьНастройки(Элемент, , "НастройкиРедактора");
...
Процедура КонструкторВыраженияВКавычкахВопросЗакрыт(Результат, ДополнительныеПараметры) Экспорт
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("КонструкторВыраженияВКавычкахЗакрыт", ЭтотОбъект, ДополнительныеПараметры);
	...
	ИначеЕсли СтрНайти(ИмяКонструктора, "КонструкторСтрокиНаРазныхЯзыках") Тогда
		...
		ИТК_ФормаКлиент.ОткрытьКонструкторСтрокНаРазныхЯзыках(Форма, ОповещениеОЗакрытии, СтрокаРезультат);
		
	ИначеЕсли СтрНайти(ИмяКонструктора, "КонструкторЗапросаВКоде") Тогда
		
		ИТК_ЗапросКлиент.ОткрытьКонструктор(СтрокаРезультат, ОповещениеОЗакрытии);
		
	КонецЕсли;		
	
КонецПроцедуры

Процедура КонструкторВыраженияВКавычкахЗакрыт(Знач Результат, ДополнительныеПараметры) Экспорт
	...
	Если НЕ ДополнительныеПараметры.ВыделенаОбласть Тогда
		...
		ЗаключатьВКавычки = НЕ ДополнительныеПараметры.ВставкаШаблона;
		Результат = ИТК_КодКлиентСервер.ЗначениеСтрока(Результат, ЗаключатьВКавычки);
...
Процедура КонструкторЗапросаЗакрыт(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не defaultView.queryMode Тогда
		Результат = ИТК_Код.Значение(Результат);
	КонецЕсли;
...
Функция ЭтоСобытиеНажатиеКлавишиEscape(Форма, ДанныеСобытия) Экспорт
	...
	ИмяСобытия = Событие.event;
	Результат = (ИмяСобытия = "EVENT_ON_KEY_ESC");
...
Процедура ПоказатьСравнение(Владелец, Текст1, Текст2, ТипЯзыка) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.ИТК_СравнениеТекстов", Параметры, Владелец, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Сравнение(Элемент, ДвустороннееСравнение, Знач Текст) Экспорт
	...
	Если Текст = "" Тогда // Нужно, иначе отключается сравнение https://github.com/salexdv/bsl_console/blob/master/docs/compare.md
		Текст = " ";
	КонецЕсли;
...
Функция ГраницыВыделения(Элемент) Экспорт
	...
	Результат = ИТК_РедакторКодаКлиентСервер.ПустойГраницыВыделения();
...
Функция УстановленныеЗакладки(Элемент) Экспорт
	...
	// Была платформенная проблема с обходом Для Каждого https://github.com/infostart-hub/toolkit/issues/1079
	Индекс = 0;
	Пока Закладки[ИТК_Строки.ЧислоВСтроку(Индекс)] <> Неопределено Цикл 
		
		НомерСтроки = Закладки[ИТК_Строки.ЧислоВСтроку(Индекс)];
...
Процедура ВосстановлениеФокусаРедактора(Форма) Экспорт
	...
	Если НЕ ИТК_РедакторКодаКлиентСервер.ЭтоРедакторКодаMonaco(ЭлементРедактора) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Элемент Из Форма.Элементы Цикл 
		...
		Если ТипЗнч(Элемент) = Тип("ТаблицаФормы")
				И ИТК_ФормаКлиент.ЭлементВиден(Элемент) Тогда
...
Процедура ПодключитьОбработчикОжиданияВосстановлениеФокуса(Форма) Экспорт
	...
	Форма.ПодключитьОбработчикОжидания(ИТК_ТуллкитКлиентСервер.Префикс() + "ПодключаемыйВосстановлениеФокусаРедактора", 0.1, Истина);
...
Процедура ПрименитьНастройкиРедактора(Элемент, Настройки) Экспорт
	...
	ПрименитьНастройкиРедактораКОбъекту(Элемент, Настройки);
...
Процедура ОбработкаОповещения(Форма, ИмяСобытия, Параметр, Источник) Экспорт
	...
	Для Каждого Элемент Из Элементы Цикл 
		...
		ПрименяемыеНастройки = ИТК_ОбщийКлиентСервер.СкопироватьОбъект(НастройкиРедактора);
		...
		ПрименитьНастройкиРедактораКОбъекту(Элемент, ПрименяемыеНастройки);
...
Функция НайтиЭлементыРедакторыMonaco(Форма) Экспорт
	...
	Для Каждого Элемент Из Элементы Цикл 
		...
		Если ТипЗнч(Элемент) = Тип("ПолеФормы")
				И ИТК_РедакторКодаКлиентСервер.ЭтоРедакторКодаMonaco(Элемент)
...
Функция ОписаниеМодуля(ПараметрыСобытия) Экспорт
	...
	Результат.Вставить("Коллекция", Неопределено);
	Результат.Вставить("КоллекцияПредставление", Неопределено);
	Результат.Вставить("ИмяОбъектаКоллекции", Неопределено);
...
Процедура ОбработатьПодсказкуВыгруженногоМодуля(Элемент, ОписаниеМодуля, ТекстМодуля) Экспорт
	...
	defaultView = ДокументHTML(Элемент);
	ИмяОбъектаВ1С = ИТК_РедакторКодаВызовСервера.ИмяОбъектаМетаданныхВ1С(ТипОбъекта, ОписаниеМодуля.ИмяОбъекта);
	Если ЗначениеЗаполнено(ТипМодуля) Тогда
		
		ИмяКоллекцииMonaco = ИТК_РедакторКодаКлиентСерверПовтИсп.ИмяКоллекцииМетаданныхПоТипу(ТипОбъекта);
...
Функция ЗаголовокВыгруженногоМодуля(ОписаниеМодуля) Экспорт
	...
	Если ОписаниеМодуля.ТипМодуля = "ManagedApplication" Тогда
		Результат = НСтр("ru = 'Модуль управляемого приложения'; en = 'Managed application module'");
	ИначеЕсли ОписаниеМодуля.ТипМодуля = "Session" Тогда
		Результат = НСтр("ru = 'Модуль сеанса'; en = 'Session module'");
	ИначеЕсли ОписаниеМодуля.ТипМодуля = "OrdinaryApplication" Тогда
		Результат = НСтр("ru = 'Модуль обычного приложения'; en = 'Regular application module'");
	ИначеЕсли ОписаниеМодуля.ТипМодуля = "ExternalConnection" Тогда
		Результат = НСтр("ru = 'Модуль внешнего соединения'; en = 'External connection module'");
	ИначеЕсли ЗначениеЗаполнено(ОписаниеМодуля.ТипМодуля) Тогда
		
		Если ОписаниеМодуля.ТипМодуля = "manager" Тогда
			ПредставлениеТипаМодуля = НСтр("ru = 'модуль менеджера'; en = 'manager module'");
		ИначеЕсли ОписаниеМодуля.ТипМодуля = "object" Тогда
			ПредставлениеТипаМодуля = НСтр("ru = 'модуль объекта'; en = 'object module'");
		ИначеЕсли ОписаниеМодуля.ТипМодуля = "valuemanager" Тогда
			ПредставлениеТипаМодуля = НСтр("ru = 'модуль менеджера значения'; en = 'value manager module'");
		ИначеЕсли ОписаниеМодуля.ТипМодуля = "command" Тогда
			ПредставлениеТипаМодуля = НСтр("ru = 'модуль команды'; en = 'command module'");
		Иначе
			ПредставлениеТипаМодуля = ОписаниеМодуля.ТипМодуля;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОписаниеМодуля.Коллекция) Тогда
			
			Результат = СтрШаблон("%1.%2.%3.%4 (%5)", ОписаниеМодуля.ТипОбъекта, ОписаниеМодуля.ИмяОбъекта,
														ОписаниеМодуля.КоллекцияПредставление, ОписаниеМодуля.ИмяОбъектаКоллекции,
														ПредставлениеТипаМодуля);
														
		Иначе
			Результат = СтрШаблон("%1.%2 (%3)", ОписаниеМодуля.ТипОбъекта, ОписаниеМодуля.ИмяОбъекта, ПредставлениеТипаМодуля);
		КонецЕсли;
...
Процедура АктивироватьСтрокуПоиска(Форма, Элемент, СтрокаПоиска) Экспорт
	...
	Начало = ИТК_РедакторКодаКлиентСервер.КурсорПоНомеруСимвола(Текст, НомерСимвола);
	
	ИТК_РедакторКодаКлиент.УстановитьГраницыВыделения(Элемент, Начало.НомерСтроки, Начало.НомерКолонки, Начало.НомерСтроки, Начало.НомерКолонки + СтрДлина(СтрокаПоиска));
...
Процедура ОткрытьКодМодуля(ИмяМодуля, ПерейтиК = Неопределено) Экспорт
	...
	ОписаниеМодуля = ИТК_ФайлыКонфигурацииКлиентСервер.ОписаниеМодуляПерейтиКОпределению(ИмяМодуля);
	
	ПутьКВыгруженнымФайлам = ИТК_ФайлыКонфигурации.ПутьКФайлам();
	ТекстМодуля = ИТК_ФайлыКонфигурации.ТекстМодуля(ПутьКВыгруженнымФайлам, ОписаниеМодуля);
	...
	ОткрытьРедактированиеКодаВОтдельномОкне(ЗаголовокФормы, ТекстМодуля, ДополнительныеПараметры);
...
Процедура ОткрытьРедактированиеКодаВОтдельномОкне(Заголовок, Код, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Владелец = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "Владелец");
	ПерейтиК = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ПерейтиК");
	Редактирование = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "Редактирование", Истина);
	ОписаниеОповещенияОЗакрытии = ИТК_ОбщийКлиентСервер.Свойство(ДополнительныеПараметры, "ОписаниеОповещенияОЗакрытии");
	...
	ОткрытьФорму("ОбщаяФорма.ИТК_РедактированиеКода", Параметры, Владелец, КлючУникальности, , , ОписаниеОповещенияОЗакрытии, РежимОткрытияОкна);
...
Процедура ОбновитьПодсказкиИспользуемыхПеременных(Элемент, Подсказки) Экспорт
	...
	ПодсказкиПеременныхJSONСтрока = ИТК_ОбщийКлиентСервер.КоллекциюВJSONСтроку(Подсказки);
	
	defaultView = ДокументHTML(Элемент);
	defaultView.setCustomHovers(ПодсказкиПеременныхJSONСтрока);	
...
Процедура Распаковать(ДополнительныеПараметры)
	...
	#Если НЕ ВебКлиент Тогда
		...
		// Разворачиваем пакет
		ЭтоWindowsКлиент = ИТК_ОбщийКлиент.ЭтоWindowsКлиент();
		ДанныеПакета = ИТК_РедакторКодаВызовСервера.ДанныеПакетаУлучшенногоРедактораКода(ЭтоWindowsКлиент);
...
Процедура ОткрытьКонструкторВыраженияВКавычках(Форма, Элемент, Имя, Текст, ВставкаШаблона = Ложь)
	
	Многострочный = Истина;
	Если СтрНайти(Имя, "КонструкторФорматнойСтроки") Тогда
		Многострочный = Ложь;
	КонецЕсли;
	Результат = СтрокаВыражениеВКавычках(Элемент, Текст, Многострочный);
	...
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("КонструкторВыраженияВКавычкахВопросЗакрыт", ЭтотОбъект, ДополнительныеПараметры);
	...
	Если СоздатьНовую Тогда
		...
		ПоказатьВопрос(ОповещениеОЗакрытии, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да, ИТК_КонсольРазработчикаКлиентСервер.Представление()); 
...
Процедура ОткрытьКонструкторВыражениеПриВставкеШаблона(Форма, Элемент, ИмяСобытия)
	...
	Текст = Текст(Форма, Элемент.Имя);
	ОткрытьКонструкторВыраженияВКавычках(Форма, Элемент, ИмяКонструктора, Текст, Истина);
...
Процедура ВставкаСсылкиНаОбъектБД(Форма, Элемент)
	...
	ВыделенныйТекст = ВыделенныйТекст(Элемент);
	ИТК_ФормаКлиент.ОткрытьВставкуСсылкиНаОбъектБД(Элемент, ОписаниеОповещения, ВыделенныйТекст);
...
Функция ОбработкаТекстаПоАлгоритму(Текст, ТипАлгоритма)
	...
	Если ТипАлгоритма = ИТК_Перечисления.АлгоритмОбработкиТекстаЗакомментировать() Тогда
	...
	ИначеЕсли ТипАлгоритма = ИТК_Перечисления.АлгоритмОбработкиТекстаУбратьКомментарии() Тогда
	...
	ИначеЕсли ТипАлгоритма = ИТК_Перечисления.АлгоритмОбработкиТекстаПереносЗапросаИзКонфигуратора() Тогда
...
Процедура ПоделитьсяЧерезСервисКода(Код, ТипЯзыка)
	...
	Если ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораЗапросы() Тогда
		РежимЗапрос = 1;
	Иначе
		РежимЗапрос = 0;
	КонецЕсли;
	...
	ТелоЗапроса = ИТК_ОбщийКлиентСервер.КоллекциюВJSONСтроку(ПараметрыЗапроса);
	...
	Попытка
		...
		КоллекцияОтвет = ИТК_ОбщийКлиентСервер.JSONСтрокуВКоллекцию(JSONОтвет);
			
		Если КоллекцияОтвет["success"] Тогда 
			Ссылка = КоллекцияОтвет["full_url"];
		Иначе
			Ошибка = СтрСоединить(КоллекцияОтвет.error, Символы.ПС);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Ссылка) Тогда
		...
		ИТК_БуферОбменаКлиент.Копировать(Ссылка);
		
		Текст = НСтр("ru = 'Ссылка на код скопирована в буфер обмена';
					|en = 'Code link copied to clipboard'");
		Пояснение = Ссылка;
		ДействиеПриНажатии = Новый ОписаниеОповещения("ОбработкаДействияПерейтиПоСсылке", ЭтотОбъект, Ссылка);
		Картинка = БиблиотекаКартинок.ИТК_КопироватьВБуферОбмена;
		Статус = СтатусОповещенияПользователя.Информация;
		
	Иначе
		
		Текст = НСтр("ru = 'Ошибка при взаимодействии с сервисом paste1c.ru';
					|en = 'Error when interacting with the service paste1c.ru'");
		Пояснение = Ошибка;
		ДействиеПриНажатии = Неопределено;
		Картинка = БиблиотекаКартинок.ИТК_ПредупреждениеВажное;
...
Процедура ТекстЗапросаКодНаВстроенномЯзыке(ТекстЗапроса)
	
	КодЯзыкаПрограммирования = ИТК_ОбщийВызовСервера.КодЯзыкаПрограммирования();
	ШаблонКода = НСтр("ru = '%1;'; en = '%1;'", КодЯзыкаПрограммирования);
	
	ТекстЗапросаКодНаВстроенномЯзыке = ИТК_Код.Значение(ТекстЗапроса);
	КодНаВстроенномЯзыке = СтрШаблон(ШаблонКода, ТекстЗапросаКодНаВстроенномЯзыке);
	
	ИТК_БуферОбменаКлиент.Копировать(КодНаВстроенномЯзыке);
	
КонецПроцедуры

Функция СтрокаВыражениеВКавычках(Элемент, Текст, Многострочный = Истина)
	
	Граница = ИТК_РедакторКодаКлиентСервер.ПустойГраницыВыделения();
	СимволГраница = ИТК_Строки.СимволКавычки();
	ГраницыВыделения = ГраницыВыделения(Элемент);
	...
	Иначе
		...
		НомерСимвола = ИТК_РедакторКодаКлиентСервер.НомерСимволаПоКурсору(Текст, НомерСтроки, НомерКолонки);
		...
		Иначе
			...
			Если КорректнаяСтрокаВКавычкахИзКода(Строка) Тогда
					
				Строка = ИТК_КодКлиентСервер.СтрокуВЗначение(Строка);
				
				КурсорНачала = ИТК_РедакторКодаКлиентСервер.КурсорПоНомеруСимвола(Текст, НомерСимволаНачало);
				Граница.НачалоСтроки = КурсорНачала.НомерСтроки;
				Граница.НачалоКолонки = КурсорНачала.НомерКолонки;
				
				КурсорКонца = ИТК_РедакторКодаКлиентСервер.КурсорПоНомеруСимвола(Текст, НомерСимволаКонец);
...
Функция КорректнаяСтрокаВКавычкахИзКода(Строка)
...
Процедура ПрименитьНастройкиРедактораКОбъекту(Элемент, Настройки)
	
	defaultView = ДокументHTML(Элемент);
	
	defaultView.setTheme(Настройки.Тема);
	defaultView.minimap(Настройки.ОтображатьКарту);
	
	ТипЯзыка = ИТК_РедакторКодаКлиентСервер.ТипЯзыка(Элемент);
	РежимЯзыкаМонако = РежимЯзыкаМонако(ТипЯзыка);
	ЭтоРежимКод = (РежимЯзыкаМонако = "bsl");
	Если ЭтоРежимКод Тогда
		
		ПутьКФайлуШаблоновКода = ИТК_ОбщийКлиентСервер.Свойство(Настройки, "ПутьКФайлуШаблоновКода");
		Если ЗначениеЗаполнено(ПутьКФайлуШаблоновКода) Тогда
			
			ТекстФайлаШаблонов = ИТК_ФайлыКлиентСервер.ПрочитатьТекст(ПутьКФайлуШаблоновКода);
...
Функция ФиксированныеНастройкиРедактора(Элемент)
	...
	ДополнительнаяИнформация = ИТК_РедакторКодаКлиентСервер.ДополнительнаяИнформация(Элемент);
	
	Результат = ИТК_ОбщийКлиентСервер.Свойство(ДополнительнаяИнформация, "ФиксированныеНастройки", Новый Структура);
	Возврат Результат;
...
Процедура ОбработкаСобытияGET_METADATA(Элемент, ПараметрыСобытия)
	...
	Если ЗначениеЗаполнено(ОписаниеМодуля) Тогда
		...
		ОбработатьПодсказкуВыгруженногоМодуля(Элемент, ОписаниеМодуля, ТекстМодуля);
	...
	ЭлементыМетаданных = ИТК_РедакторКодаВызовСервера.ОбработкаСобытияПолученияМетаданных(МетаданныеСобытия);
...
Функция РежимЯзыкаМонако(ТипЯзыка)
	...
	Если ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораВстроенный() Тогда
		
		Результат = "bsl";
		
	ИначеЕсли ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораЗапросы() Тогда
		
		Результат = "bsl_query";
		
	ИначеЕсли ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораXML() Тогда
		
		Результат = "xml";
		
	ИначеЕсли ТипЯзыка = ИТК_Перечисления.ТипЯзыкаРедактораВыраженияСКД() Тогда
