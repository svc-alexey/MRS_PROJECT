...
Функция ФильтрФайловДанных(ВключаяВсеФайлы = Ложь, ВключатьИмпортируемыеФорматы = Ложь) Экспорт
	...
	ПоддерживаемыеФорматы.Добавить(СтрШаблон("*.%1", ОсновноеРасширениеФайловДанных()), НСтр("ru = 'Файлы данных';
																							|en = 'Data files'"));
...
Функция ФильтрФайлов(Форматы, ВключаяВсеФайлы = Истина) Экспорт
	...
	Если Форматы.Количество() > 1 Тогда
		Фильтры.Добавить(СтрокаФильтраФайлов(НСтр("ru = 'Все поддерживаемые форматы';
	...
	Если ВключаяВсеФайлы Тогда
		Фильтры.Добавить(СтрокаФильтраФайлов(НСтр("ru = 'Все файлы';
...
Функция ПредставлениеОшибкиВТекстеЗапросаИлиСхемеКомпоновкиДанных(Ошибка) Экспорт
	...
	Иначе
		...
		Результат = СтрШаблон("(%1, %2) %3", ИТК_Строки.ЧислоВСтроку(Ошибка.НомерСтроки), Ошибка.НомерСтолбца, Ошибка.Текст);
...
Функция ЭтоСсылка(Ссылка, ИсключаяПеречисления = Ложь) Экспорт
	...
	Иначе
		...
		Результат = ИТК_ТипыКлиентСервер.ЭтоСсылочный(ТипЗнч(Ссылка), ИсключаяПеречисления);
...
Функция ЭтоЗначениеПараметраГраница(Значение) Экспорт
	...
	Возврат ТипЗнч(Значение) = Тип("Структура") 
				И Значение.Вид = ИТК_Перечисления.СложныйПараметрЗапросаГраница();
...
//
// Параметры:
//  Режим  - Перечисление.ИТК_РежимВыполненияЗапроса, Перечисление.ИТК_РежимВыполненияСхемыКомпоновкиДанных - Режим выполнения
//
// Возвращаемое значение:
...
Функция ЭтоРежимВыполненияСРезультатом(Режим) Экспорт
	...
	Возврат Режим = ИТК_Перечисления.РежимВыполненияЗапросаВыполнение()
		ИЛИ Режим = ИТК_Перечисления.РежимВыполненияЗапросаСРезультатамиВременныхТаблиц()
		ИЛИ Режим = ИТК_Перечисления.РежимВыполненияСКДВыполнение()
		ИЛИ Режим = ИТК_Перечисления.РежимВыполненияКодаВыполнение()
...
Функция ПредставлениеВремениВМс(ЧислоМиллисекунд) Экспорт
	...
	ИначеЕсли ЧислоМиллисекунд = 0 Тогда
		...
		Результат = НСтр("ru = '< 1 мс';
						|en = '< 1 ms'");
		
	ИначеЕсли ЧислоМиллисекунд < 100 Тогда
		
		Результат = СтрШаблон(НСтр("ru = '%1 мс';
									|en = '%1 ms'"), ЧислоМиллисекунд);
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ВремяМинут) Тогда
			ЭлементыПредставления.Добавить(СтрШаблон(НСтр("ru = '%1 мин.';
															|en = '%1 min.'"), Формат(ВремяМинут, "ЧГ=")));
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ВремяВСекундах) Тогда
			ЭлементыПредставления.Добавить(СтрШаблон(НСтр("ru = '%1 с.';
															|en = '%1 s.'"), ВремяВСекундах));
		КонецЕсли;
...
Функция ПроцентОтОбщего(Число, Всего, Точность = 2) Экспорт
	...
	Возврат Окр((Число * 100) / Всего, Точность);
...
Функция ПоддерживаетсяПлатформойИРежимомСовместимости(Версия) Экспорт 
	
	Возврат (СравнениеВерсииПлатформыСТекущей(Версия, Истина) <> -1);
...
Функция МассивВСписокЗначений(Значение) Экспорт
	
	Результат = Новый СписокЗначений;
	Результат.ЗагрузитьЗначения(Значение);
	
	Возврат Результат;
	
КонецФункции

Функция СоответствиеВСписокЗначений(Значение) Экспорт
	
	Результат = Новый СписокЗначений;
	Для Каждого КлючЗначение Из Значение Цикл
		Результат.Добавить(КлючЗначение.Значение, КлючЗначение.Ключ);
	КонецЦикла;
...
Функция СписокЗначенийВМассив(Значение) Экспорт
...
Функция ИнформационнаяБазаФайловая() Экспорт 
	...
	Возврат ИТК_ОбщийКлиентСерверПовтИсп.ИнформационнаяБазаФайловая();
	
КонецФункции

Функция ИмяРеквизитаФормыОбъект() Экспорт
...
Функция ИмяРеквизитаРасшифровкиОтчета() Экспорт
	
	Если ИТК_ОбщийКлиентСерверПовтИсп.ВариантВстроенногоЯзыкаАнглийский() Тогда
		Результат = "ReportDetailsData1";
	Иначе
		Результат = "ОтчетДанныеРасшифровки";
	КонецЕсли;
...
Функция ИмяКоллекцииТекущиеДанные() Экспорт
	
	Если ИТК_ОбщийКлиентСерверПовтИсп.ВариантВстроенногоЯзыкаАнглийский() Тогда
		Результат = "CurrentData";
	Иначе
...
Функция ПостфиксТипаСсылка() Экспорт
...
Функция ПостфиксТипаОбъект() Экспорт
...
Функция ПостфиксТипаМенеджер() Экспорт
...
Функция ПостфиксТипаНаборЗаписей() Экспорт
	
	Если ИТК_ОбщийКлиентСерверПовтИсп.ВариантВстроенногоЯзыкаАнглийский() Тогда
		Результат = "RecordSet";
	Иначе
		Результат = "НаборЗаписей";
	КонецЕсли;
...
Функция ПостфиксТипаМенеджерЗначения() Экспорт
	
	Если ИТК_ОбщийКлиентСерверПовтИсп.ВариантВстроенногоЯзыкаАнглийский() Тогда
		Результат = "ValueManager";
	Иначе
		Результат = "МенеджерЗначения";
	КонецЕсли;
...
Функция ПредставлениеСочетанияКлавиш(СочетаниеКлавиш) Экспорт
	...
	Если СочетаниеКлавиш.Ctrl Тогда
		ЧастиПредставления.Добавить("Ctrl");
	КонецЕсли;
	
	Если СочетаниеКлавиш.Alt Тогда
		ЧастиПредставления.Добавить("Alt");
	КонецЕсли;
	
	Если СочетаниеКлавиш.Shift Тогда
		ЧастиПредставления.Добавить("Shift");
	КонецЕсли;
	
	ОсобыеПредставленияКлавишы = ИТК_ОбщийКлиентСерверПовтИсп.ОсобыеПредставленияКлавишы();
	ОсобоеПредставление = ОсобыеПредставленияКлавишы.Получить(СочетаниеКлавиш.Клавиша);
	Если ЗначениеЗаполнено(ОсобоеПредставление) Тогда
		ЧастиПредставления.Добавить(ОсобоеПредставление);
	Иначе
		ЧастиПредставления.Добавить(СочетаниеКлавиш.Клавиша);
	КонецЕсли;
...
Функция СравнениеВерсииПлатформыСТекущей(ВерсияСравнения, РежимСовместимости = Ложь)

	Если РежимСовместимости Тогда
		ТекущаяВерсия = ИТК_ОбщийВызовСервера.ВерсияРежимаСовместимости();
	Иначе
	...
	Возврат СравнитьВерсии(ТекущаяВерсия, ВерсияСравнения);
