...
Процедура ПрочитатьСобытияЖурналаРегистрации(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Если ДатыОтбораУказаны И ДатаНачала > ДатаОкончания Тогда
		ВызватьИсключение НСтр("ru = 'Некорректно заданы условия отбора журнала регистрации. Дата начала больше даты окончания.';
								|en = 'Incorrect conditions for selecting the registration log. The start date is greater than the end date.'");
	КонецЕсли;
	...
	Данные = ИТК_ОбщийКлиентСервер.Свойство(Отбор, "Данные");
	Если ТипЗнч(Данные) = Тип("СписокЗначений") Тогда
		
		ОтборДанных = Отбор.Данные;
		Отбор.Удалить("Данные");
		ЭтоОтборДанныхПоОбъекту = Истина;
		
	Иначе
		
		ОтборДанных = Неопределено;
		ЭтоОтборДанныхПоОбъекту = Ложь;
	...
	Если ЭтоОтборДанныхПоОбъекту Тогда
		
		КоличествоСобытийПоОбъекту = КоличествоСобытий;
		Для Каждого ЭлементОтборДанных Из ОтборДанных Цикл 
			
			СобытияЖурналаПоОбъекту = Новый ТаблицаЗначений;
			ОтборПоОбъекту = ИТК_ОбщийКлиентСервер.СкопироватьОбъект(Отбор);
			ОтборПоОбъекту.Вставить("Данные", ЭлементОтборДанных.Значение);
			
			ВыгрузитьЖурналРегистрации(СобытияЖурналаПоОбъекту, ОтборПоОбъекту, , , КоличествоСобытийПоОбъекту);
			
			Если ЗначениеЗаполнено(СобытияЖурнала) Тогда
				
				Для Каждого Строка Из СобытияЖурналаПоОбъекту Цикл 
			...
			Иначе
				СобытияЖурнала = СобытияЖурналаПоОбъекту.Скопировать();
			КонецЕсли;
			КоличествоСобытийПоОбъекту = КоличествоСобытийПоОбъекту - СобытияЖурналаПоОбъекту.Количество();
	...
	Если ИТК_БСППовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
	...
	Если ИТК_БСППовтИсп.РазделениеВключено()
			И ИТК_БСППовтИсп.ДоступноИспользованиеРазделенныхДанных()
			И ИТК_БСППовтИсп.СуществуетПодсистема("ТехнологияСервиса") Тогда
	...
	Для Каждого СобытиеЖурнала Из СобытияЖурнала Цикл
		...
		Если ИТК_БСППовтИсп.РазделениеВключено()
			И Не ИТК_БСППовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ИначеЕсли СобытиеЖурнала.Событие = "_$Access$_.AccessDenied" Тогда
			...
			Если СобытиеЖурнала.Данные <> Неопределено Тогда
				Если СобытиеЖурнала.Данные.Свойство("Право") Тогда
					...
					СобытиеЖурнала.Данные = СтрШаблон(
												НСтр("ru = 'Право: %1'; en = 'Right: %1'"),
				...
				Иначе
					...
					СобытиеЖурнала.Данные = СтрШаблон(
												НСтр("ru = 'Действие: %1%2'; en = 'Action: %1%2'"),
												ДанныеЖурнала.Действие,
		...
		ИначеЕсли СобытиеЖурнала.Событие = "_$User$_.New"
			...
			КонецЕсли;
			СобытиеЖурнала.Данные = СтрШаблон(НСтр("ru = 'Имя: %1, ...'; en = 'Name: %1, ...'"), ИмяПользователяИБ);
		...
		Если СобытиеЖурнала.Пользователь = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000") Тогда
			СобытиеЖурнала.ИмяПользователя = НСтр("ru = '<Неопределен>'; en = '<Undefined>'");
		ИначеЕсли СобытиеЖурнала.ИмяПользователя = "" Тогда
			СобытиеЖурнала.ИмяПользователя = НСтр("ru = '<Не указан>'; en = '<Unspecified>'");
		ИначеЕсли ПользователиИнформационнойБазы.НайтиПоУникальномуИдентификатору(СобытиеЖурнала.Пользователь) = Неопределено Тогда
			СобытиеЖурнала.ИмяПользователя = СтрШаблон(НСтр("ru = '%1 <Удален>'; en = '%1 <Deleted>'"), СобытиеЖурнала.ИмяПользователя);
		КонецЕсли;
		
		Если ПсевдонимыПользователей <> Неопределено Тогда
			
			Если ПсевдонимыПользователей.Получить(СобытиеЖурнала.Пользователь) = Неопределено Тогда
				
				ПараметрыФункции = Новый Массив;
				ПараметрыФункции.Добавить(СобытиеЖурнала.Пользователь);
				ПсевдонимПользователя = ИТК_БСПКлиентСервер.ВыполнитьФункцию("РаботаВМоделиСервиса.ПсевдонимПользователяИнформационнойБазы",
																				 ПараметрыФункции);
...
Процедура ПреобразованиеЭлементаОтбора(Отбор, ЭлементОтбора)
	...
	Для Каждого ЗначениеИзСписка Из ЭлементОтбора.Значение Цикл
		Если ВРег(КлючСтруктурыОтбора) = ВРег("Уровень") Тогда
			...
			НовоеЗначение.Добавить(Обработки.ИТК_ЖурналРегистрации.УровеньЖурналаРегистрацииЗначениеПоИмени(ЗначениеИзСписка.Значение));
		ИначеЕсли ВРег(КлючСтруктурыОтбора) = ВРег("СтатусТранзакции") Тогда
			...
			НовоеЗначение.Добавить(Обработки.ИТК_ЖурналРегистрации.СтатусТранзакцииЗаписиЖурналаРегистрацииЗначениеПоИмени(ЗначениеИзСписка.Значение));
...
Процедура ДобавитьОграничениеВПредставлениеОтбора(ОтборЖурналаРегистрации, ПредставлениеОтбора, ИмяОграничения,
	...
	Если ИмяОграничения = "Событие" И СписокОграничений.Количество() > 5 Тогда
		...
		Ограничение = СтрШаблон(НСтр("ru = 'События (%1)'; en = 'Events (%1)'"), СписокОграничений.Количество());
		
	ИначеЕсли ИмяОграничения = "Сеанс" И СписокОграничений.Количество() > 3 Тогда
		
		Ограничение = СтрШаблон(НСтр("ru = 'Сеансы (%1)'; en = 'Sessions (%1)'"), СписокОграничений.Количество());
		...
		Для Каждого ЭлементСписка Из СписокОграничений Цикл
			...
			Если (ВРег(ИмяОграничения) = ВРег("Сеанс")
				...
				Ограничение = НСтр("ru = '%1: %2';
									|en = '%1: %2'");
				Ограничение = СтрШаблон(Ограничение, ПредставлениеОграничения, ЗначениеОграничения);
			Иначе
