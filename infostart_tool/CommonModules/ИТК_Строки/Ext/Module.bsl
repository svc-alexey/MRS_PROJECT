...
Функция КраткоеПредставлениеИнтервалаВремени(КоличествоСекунд) Экспорт
	...
	Если КоличествоСекунд = 0 Тогда
		Возврат НСтр("ru = 'меньше секунды';
	...
	Если ЗначениеЗаполнено(Месяцев) Тогда
		Результат.Добавить(СтрШаблон(НСтр("ru = '%1 мес.';
											|en = '%1 m.'"), Месяцев));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Недель) Тогда
		Результат.Добавить(СтрШаблон(НСтр("ru = '%1 нед.';
											|en = '%1 w.'"), Недель));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Дней) Тогда
		Результат.Добавить(СтрШаблон(НСтр("ru = '%1 дн.';
											|en = '%1 d.'"), Дней));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Часов) Тогда
		Результат.Добавить(СтрШаблон(НСтр("ru = '%1 ч.';
											|en = '%1 h.'"), Часов));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Минут) Тогда
		Результат.Добавить(СтрШаблон(НСтр("ru = '%1 мин.';
											|en = '%1 min.'"), Минут));
	КонецЕсли;
	...
	Если КоличествоСекунд < 15 * 60 Тогда
		...
		Если ЗначениеЗаполнено(ОстатокИнтервала) Тогда
			...
			Результат.Добавить(СтрШаблон(НСтр("ru = '%1 сек.';
												|en = '%1 sec.'"), Окр(ОстатокИнтервала, Разрядность)));
...
Функция ПроверкаИдентификатора(Идентификатор, ЗанятыеИдентификаторы = Неопределено, ДополнительныеСимволы = "") Экспорт
	...
	Если ИдентификаторИспользуется Тогда
		Ошибки.Добавить(НСтр("ru = 'Идентификатор используется';
							|en = 'ID used'"));
	КонецЕсли;
...
Функция ПроверкаИдентификатораСтрока(Идентификатор, ЗанятыеИдентификаторы = Неопределено) Экспорт
	...
	Если Идентификатор = "" Тогда
		Ошибки.Добавить(НСтр("ru = 'Пустой идентификатор';
							|en = 'Empty id'"));
	КонецЕсли;
...
Функция ЭтоПредставлениеБитаяСсылка(Строка) Экспорт
	...
	Возврат СтрНачинаетсяС(Строка, НСтр("ru = '<Объект не найден>';
										|en = '<Object not found>'"));
	
КонецФункции

Функция ЭтоНавигационнаяСсылкаДанные(Строка) Экспорт
	
	Возврат Булево(СтрНайти(Строка, МаркерНавигационнойСсылкиДанные()));
	
КонецФункции

Функция МаркерНавигационнойСсылкиДанные() Экспорт
...
Функция ПараметрыПоискаПоПодобию(СтрокаПоиска) Экспорт
	
	ЧастиМетаданныеКоллекция = СтрРазделить(НРег(СтрокаПоиска), "/");
	
	Коллекция = Неопределено;
	Если ЧастиМетаданныеКоллекция.Количество() > 1 Тогда
		
		ЧастиКоллеций = СтрРазделить(ЧастиМетаданныеКоллекция[1], " ");
		Коллекция = СокрЛП(ЧастиКоллеций[0]);
		
	КонецЕсли;
	
	ЧастиИмениМетаданных = СтрРазделить(ЧастиМетаданныеКоллекция[0], " ");
	Слово = СокрЛП(ЧастиИмениМетаданных[0]);
	
	ДополнительныеСлова = Новый Массив;
	Для Индекс = 1 По ЧастиИмениМетаданных.ВГраница() Цикл 
		
		ДополнительноеСлово = СокрЛП(ЧастиИмениМетаданных[Индекс]);
		Если ЗначениеЗаполнено(ДополнительноеСлово) Тогда
			ДополнительныеСлова.Добавить(ДополнительноеСлово);
		КонецЕсли;
		
	КонецЦикла;
	
	Результат = Новый Структура;
	Результат.Вставить("СтрокаПоиска", НРег(СтрокаПоиска));
	Результат.Вставить("Коллекция", Коллекция);
	Результат.Вставить("Слово", Слово);
	Результат.Вставить("ДополнительныеСлова", ДополнительныеСлова);
	
	Возврат Результат;
	
КонецФункции

Функция ПоискПоПодобиюНайденыДополнительныеСлова(АнализируемаяСтрока, ПараметрыПоиска) Экспорт
	
	Для Каждого ДополнительноеСлово Из ПараметрыПоиска.ДополнительныеСлова Цикл 
		
		Если НЕ СтрНайти(АнализируемаяСтрока, ДополнительноеСлово) Тогда
			Возврат Ложь;
		КонецЕсли;
...
Функция ПоискПоПодобиюНайденыСовпадения(АнализируемаяСтрока, ПараметрыПоиска) Экспорт
	
	АнализируемаяСтрока = НРег(АнализируемаяСтрока);
	
	Если НЕ СтрНайти(АнализируемаяСтрока, ПараметрыПоиска.Слово) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ИТК_Строки.ПоискПоПодобиюНайденыДополнительныеСлова(АнализируемаяСтрока, ПараметрыПоиска);
...
Функция ТекстЗаданиеВыполняется() Экспорт
	...
	Возврат НСтр("ru = 'Задание выполняется';
				|en = 'Task in progress'");
...
Функция ТекстЗаданиеЗавершеноСОшибками() Экспорт
	...
	Возврат НСтр("ru = 'Задание завершено с ошибками';
				|en = 'Job completed with errors'");
...
Функция НайтиИВыделитьОформлением(Строка, СтрокаПоиска) Экспорт
	
	Если ИТК_ОбщийКлиентСервер.ПоддерживаетсяПлатформой("8.3.15") Тогда
		
		Результат = Вычислить("СтрНайтиИВыделитьОформлением(Строка, СтрокаПоиска)");
	...
	ЧастиПоиска = ЧастиПоискаПоПодобию(СтрокаПоиска);
...
Функция СимволКавычки() Экспорт
	
	Возврат """";
	
КонецФункции

Функция СимволДвойныеКавычки() Экспорт
	
	Возврат СимволКавычки() + СимволКавычки();
...
Функция СтрокаВДату(ДатаСтрока) Экспорт
	...
	ДатаПоЧастям = СтрРазделить(ДатаСтрока, " ");
	
	ОсновнаяДата = ДатаПоЧастям[0];
	ЧастиОсновнойДаты = СтрРазделить(ОсновнаяДата, ".");
	
	Если ЧастиОсновнойДаты.Количество() = 3 Тогда
		
		ЧастьГод = ЧастиОсновнойДаты[2];
		Если СтрДлина(ЧастьГод) = 2 Тогда
			ЧастьГод = "20" + ЧастьГод;
		КонецЕсли;
		
		ДатаЛитерал = ЧастьГод + ЧастиОсновнойДаты[1] + ЧастиОсновнойДаты[0];
		
		// Время
		Если ДатаПоЧастям.Количество() = 2 Тогда
			
			ЧастиВремени = СтрРазделить(ДатаПоЧастям[1], ":");
			Для Каждого ЧастьВремени Из ЧастиВремени Цикл
				
				Если СтрДлина(ЧастьВремени) < 2 Тогда
					ЧастьВремени = "0" + ЧастьВремени;
				КонецЕсли;
				
				ДатаЛитерал = ДатаЛитерал + ЧастьВремени;
	...
	Иначе
		ДатаЛитерал = Неопределено;
	КонецЕсли;
	
	ОписаниеТипа = Новый ОписаниеТипов("Дата");
	Результат = ОписаниеТипа.ПривестиЗначение(ДатаЛитерал);
