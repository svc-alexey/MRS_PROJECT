...
Функция ВычислитьВыражениеНаВстроенномЯзыке(Алгоритм, Комментарий) Экспорт
	...
	Исключение
		...
		|%2
		|Ошибка: %3';
		|en = 'The computation of the expression (%1):
		|%2
		|Error: %3'"), Комментарий, Алгоритм, ПредставлениеОшибки);
...
Функция КопияТаблицыЗначений(Адрес, УникальныйИдентификатор) Экспорт
	...
	Если ЭтоАдресВременногоХранилища(Адрес) Тогда
		...
		Результат = ИТК_ВременноеХранилище.Поместить(ТаблицаЗначений.Скопировать(), , УникальныйИдентификатор);
...
Функция ИнформационнаяБазаФайловая() Экспорт
	...
	Возврат ИТК_ОбщийКлиентСервер.ИнформационнаяБазаФайловая();
...
Функция ИнформацияДляТехническойПоддержки() Экспорт
	...
	Возврат ИТК_Общий.ИнформацияДляТехническойПоддержки();
	
КонецФункции

Функция КодЯзыкаПрограммирования() Экспорт
	
	Возврат ИТК_Общий.КодЯзыкаПрограммирования();
	
КонецФункции

Функция ВерсияРежимаСовместимости() Экспорт
	
	Возврат ИТК_ОбщийПовтИсп.ВерсияРежимаСовместимости();
