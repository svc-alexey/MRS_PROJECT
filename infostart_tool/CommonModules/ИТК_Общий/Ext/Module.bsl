...
Функция КодЯзыкаПрограммирования() Экспорт
	...
	Результат = ИТК_ОбщийКлиентСерверПовтИсп.КодЯзыкаПрограммированияКонфигурации();
...
//
Процедура ВыполнитьПроцедуру(Знач ИмяМетода, Знач Параметры, БезопасныйРежим = Истина) Экспорт
...
Функция ВыполнитьФункцию(Знач ИмяМетода, Знач Параметры, БезопасныйРежим = Истина) Экспорт
	...
	// BSLLS:ExecuteExternalCodeInCommonModule-off
	Результат = Вычислить(ИмяМетода + "(" + СтрСоединить(СписокПараметров, ", ") + ")");
	Возврат Результат;
	// BSLLS:ExecuteExternalCodeInCommonModule-on
...
Функция БитаяСсылка(Ссылка) Экспорт 
	...
	Иначе
		...
		Поле = ИТК_МетаданныеКлиентСерверПовтИсп.СтандартноеИмяПоляОбъекта("Ссылка");
		Результат = (ЗначениеРеквизитаОбъекта(Ссылка, Поле) = Неопределено);
...
Функция СсылкаПоНавигационнойСсылке(Строка) Экспорт
	...
	Попытка
		...
		Маркер = ИТК_Строки.МаркерНавигационнойСсылкиДанные();
...
Функция ЗначениеРеквизитаОбъекта(Ссылка, Знач ИмяРеквизита) Экспорт
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты) Экспорт
	...
	ШаблонТекстаЗапроса = "ВЫБРАТЬ Таблица.%1 ИЗ %2 КАК Таблица ГДЕ Таблица.Ссылка = &Ссылка";
	...
	Если ЗначениеЗаполнено(ТаблицаЗначений) Тогда
		ЗаполнитьЗначенияСвойств(Результат, ТаблицаЗначений[0]);
	КонецЕсли;
...
Функция ИнформацияДляТехническойПоддержки() Экспорт
	...
	Если ИТК_ОбщийКлиентСервер.ИнформационнаяБазаФайловая() Тогда
		Режим = НСтр("ru = 'файловый';
					|en = 'file'");
	Иначе
		Режим = НСтр("ru = 'клиент-серверный';
					|en = 'client-server'");
	КонецЕсли;
	Шаблон = НСтр("ru = 'Режим БД: %1';
					|en = 'Mode IB: %1'");
	...
	Результат.Добавить(НСтр("ru = 'Конфигурация:';
							|en = 'Configuration:'"));
	...
	Шаблон = НСтр("ru = 'Основной режим запуска: %1';
					|en = 'Basic launch mode: %1'");
	Текст = СтрШаблон(Шаблон, Метаданные.ОсновнойРежимЗапуска);
	Результат.Добавить(Текст);
	
	// Режим совместимости
	Шаблон = НСтр("ru = 'Режим совместимости: %1';
					|en = 'Compatibility mode: %1'");
	Текст = СтрШаблон(Шаблон, Метаданные.РежимСовместимости);
	Результат.Добавить(Текст);
	
	// Версия БСП
	Шаблон = НСтр("ru = 'Версия БСП: %1';
					|en = 'SSL version: %1'");
	ВерсияБСП = ИТК_БСППовтИсп.Версия();
	Если ВерсияБСП = Неопределено Тогда
		ВерсияБСП = НСтр("ru = 'БСП не установлена';
						|en = 'SSL not installed'");
	КонецЕсли;
...
Функция КэшВыделенияОсобыхЗначений(ТипыЗначений) Экспорт
	...
	Результат.Вставить("", НСтр("ru = '<Пустая строка>';
								|en = '<Empty string>'"));
	Результат.Вставить(ИТК_ОбщийКлиентСервер.ПустаяДата(), НСтр("ru = '<Пустая дата>';
																|en = '<Empty date>'"));
	
	ПустойУникальныйИдентификатор = Новый УникальныйИдентификатор(ИТК_Строки.ПустойУникальныйИдентификатор());
	Результат.Вставить(ПустойУникальныйИдентификатор, НСтр("ru = '<Пустой уникальный идентификатор>';
															|en = '<Unique identifier empty>'"));	
	...
	Результат.Вставить(Неопределено, НСтр("ru = '<Неопределено>';
											|en = '<Undefined>'"));
...
Функция КоличествоСтрокВДеревеЗначений(Строка) Экспорт
	
	Результат = 0;
	Для Каждого ПодчиненнаяСтрока Из Строка.Строки Цикл 
		
        Результат = Результат + 1;
		Результат = Результат + КоличествоСтрокВДеревеЗначений(ПодчиненнаяСтрока);
...
Функция ЗаполненоДерево(Дерево) Экспорт
	
	Если Дерево = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ЗначениеЗаполнено(Дерево.Строки);
	
КонецФункции

Функция ПредставлениеИндексирования(Индексирование) Экспорт
	
	Если Индексирование = Метаданные.СвойстваОбъектов.Индексирование.Индексировать Тогда
		Результат = НСтр("ru = 'Индексировать'; en = 'Index'");
	ИначеЕсли Индексирование = Метаданные.СвойстваОбъектов.Индексирование.ИндексироватьСДопУпорядочиванием Тогда
		Результат = НСтр("ru = 'Индексировать с доп. упорядочиванием'; en = 'Index with add ordering'");
	Иначе
		Результат = Неопределено;
	КонецЕсли;
...
Процедура УдалитьСтрокиТаблицыЗначений(ТаблицаЗначений, Отбор) Экспорт
...
Процедура ВызовСервераВКонтекстеФормы(Форма, ПолноеИмяМетода, Параметры = Неопределено) Экспорт
	
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(Форма);
	ИТК_ОбщийКлиентСервер.ДополнитьМассив(ПараметрыМетода, Параметры);
	
	ИТК_Общий.ВыполнитьПроцедуру(ПолноеИмяМетода, ПараметрыМетода);
	
КонецПроцедуры

Функция ВызовФункцииСервераВКонтекстеФормы(Форма, ПолноеИмяМетода, Параметры = Неопределено) Экспорт
	
	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(Форма);
	ИТК_ОбщийКлиентСервер.ДополнитьМассив(ПараметрыМетода, Параметры);
	
	Возврат ИТК_Общий.ВыполнитьФункцию(ПолноеИмяМетода, ПараметрыМетода);
...
Функция НайтиТипПоВнутреннемуИндексу(Индекс)
	...
	ТипСсылка = ИТК_МетаданныеКлиентСервер.ПолноеИмяТипаСсылка(ИмяОбъектаКоллекции, ОбъектМетаданных.Имя);
