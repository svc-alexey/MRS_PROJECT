...
Процедура Обработать(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	ОбработкаВТранзакции = (ОписаниеАлгоритма.Транзакция = ИТК_Перечисления.ТипТранзакцииОбработкиРезультатаНаВсе());
	...
	ИдентификаторСтрокиRU = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторСтроки("ru");
	ИдентификаторСтрокиEN = ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторСтроки("en");
	Контекст = Новый Структура;
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторПараметры("ru"), ЗначенияПараметров);
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторПараметры("en"), ЗначенияПараметров);
	Контекст.Вставить(ИдентификаторСтрокиRU);
	Контекст.Вставить(ИдентификаторСтрокиEN);
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторДополнительныеСвойства("ru"), ДополнительныеСвойства);
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторДополнительныеСвойства("en"), ДополнительныеСвойства);
	...
	Для ИндексСтроки = 0 По КоличествоСтрок - 1 Цикл  
		...
		ИТК_ДлительныеОперацииКлиентСервер.СообщениеНачалоОбработкиОбъекта(ИндексОбрабатываемойСтроки);
...
Функция ПредставлениеОбъектаЖурнала(ОбрабатываемыеДанные, Индекс) Экспорт
	...
	Результат = ИТК_ОбработкаРезультатаКлиентСервер.ПредставлениеИндексаОбъекта(Индекс);
	Если Не ИТК_ОбработкаРезультатаКлиентСервер.ЭтоИндексОбъекта(Индекс) Тогда
	...
	Если ТипЗнч(ОбрабатываемыеДанные) = Тип("Массив") Тогда
		...
		Если ИТК_МетаданныеКлиентСервер.ЭтоТипКлючЗаписиРегистраСведений(ТипЗнч(Объект)) Тогда
			Результат = ИТК_Метаданные.ПредставлениеКлючаЗаписейРегистраСведений(Объект);
		Иначе
	...
	Иначе
		...
		Представление = ИТК_Строки.Сократить(Значение, 100);
...
Функция ВыполнитьКодОбработки(Код, ЗначенияПараметров, ДополнительныеСвойства, ОбрабатываемыеДанные = Неопределено) Экспорт
	...
	Контекст = Новый Структура;
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторПараметры(), ЗначенияПараметров);
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторПараметры("en"), ЗначенияПараметров);
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторДополнительныеСвойства(), ДополнительныеСвойства);
	Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторДополнительныеСвойства("en"), ДополнительныеСвойства);
	
	Если ОбрабатываемыеДанные <> Неопределено Тогда
		Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторОбрабатываемыхДанных(), ОбрабатываемыеДанные);
		Контекст.Вставить(ИТК_ОбработкаРезультатаКлиентСервер.ИдентификаторОбрабатываемыхДанных("en"), ОбрабатываемыеДанные);
	КонецЕсли;
	
	АлгоритмРаботыСКонтекстом = АлгоритмРаботыВКонтексте(Контекст);
		
	Результат = ИТК_Общий.ВыполнитьСКонтекстом(АлгоритмРаботыСКонтекстом, Код, Контекст);
...
Функция ОбработатьСтроку(ОбрабатываемаяСтрока, АлгоритмРаботыСКонтекстом, ОписаниеАлгоритма, Контекст)
	...
	ОбработкаВТранзакции = (ОписаниеАлгоритма.Транзакция = ИТК_Перечисления.ТипТранзакцииОбработкиРезультатаПоСтроке());
	
	Если ОбработкаВТранзакции Тогда
		
		НачатьТранзакцию();
		Результат = ИТК_Общий.ВыполнитьСКонтекстом(АлгоритмРаботыСКонтекстом, ОписаниеАлгоритма.Код, Контекст);
