...
Функция Список(ВидИнструмента = Неопределено) Экспорт
	...
	Если ВидИнструмента = Неопределено Тогда
		...
		Если ИТК_ОбщийКлиентСерверПовтИсп.ВариантВстроенногоЯзыкаРусский() Тогда
		...
		Результат.Вставить("РедакторТема", ИТК_РедакторКодаКлиентСервер.ТемаКонфигуратор());
		...
		Результат.Вставить("РедакторПодключитьOneDebugger", Ложь);
	...
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментКонсольРазработчика() Тогда
		...
		Результат.Вставить("ФоновоеВыполнение", Ложь);
		
		Результат.Вставить("ОформлятьОсобыеЗначенияВРезультате", Истина);
		...
		Результат.Вставить("ВопросПередВыполнениемЗапросаСОшибкойБольшеНеПоказывать", Ложь);
	...
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментРегламентныеИФоновыеЗадания() Тогда
		
		ИТК_ИзбранноеКлиентСервер.ИнициализацияНастроек(Результат, ВидИнструмента);
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментПользователи() Тогда
		
		ИТК_ИзбранноеКлиентСервер.ИнициализацияНастроек(Результат, ВидИнструмента);
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментРедакторКонстант() Тогда
		
		ИТК_ИзбранноеКлиентСервер.ИнициализацияНастроек(Результат, ВидИнструмента);
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментРедакторПараметровСеанса() Тогда
		
		ИТК_ИзбранноеКлиентСервер.ИнициализацияНастроек(Результат, ВидИнструмента);
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ГруппаИнструментовКонсоли() Тогда
		
		Результат.Вставить("Автоcохранение", Истина);
		Результат.Вставить("СохранениеПередВыполнением", Истина);
		
		Результат.Вставить(ИмяНастройкиСохранятьДанныеПоУмолчаниюВ(), ИТК_Перечисления.ТипХраненияДанныхФайл());
		Результат.Вставить("ВопросПередУдалениемСтрокДанных", Истина);
...
Функция КлючНастроек(ВидИнструмента = Неопределено) Экспорт
	...
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментРегламентныеИФоновыеЗадания() Тогда
		...
		Результат = "Регламентные и фоновые задания (настройки)";
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментПользователи() Тогда
		
		Результат = "Пользователи ИБ (настройки)";
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментРедакторКонстант() Тогда
		
		Результат = "Редактор констант (настройки)";
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ИнструментРедакторПараметровСеанса() Тогда
		
		Результат = "Редактор параметров сеанса (настройки)";
		
	ИначеЕсли ВидИнструмента = ИТК_Перечисления.ГруппаИнструментовКонсоли() Тогда
		
		Результат = ИТК_КонсолиКлиентСерверПовтИсп.КлючНастроек()
...
Функция КлючОбъектаСлужебные(Инструмент) Экспорт
	
	КодЯзыкаПрограммированияКонфигурации = ИТК_ОбщийКлиентСерверПовтИсп.КодЯзыкаПрограммированияКонфигурации();
	Имя = НСтр("ru = 'служебные'; en = 'service'", КодЯзыкаПрограммированияКонфигурации);
	
	Результат = КлючОбъекта(Инструмент, Имя);
	Возврат Результат;
...
Функция КлючОбъекта(Инструмент, Имя)
	...
	Результат = СтрШаблон("%1 - %2 (%3)", ПредставлениеРасширения, ПредставлениеИнструмента, Имя);
