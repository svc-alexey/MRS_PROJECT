...
Функция ОписаниеШаблонов(Язык) Экспорт
	...
	Имя = НСтр("ru = 'ВыгрузитьЗапрос';
				|en = 'UploadQuery'", Язык);
	Подсказка = НСтр("ru = 'Выгрузить результат запроса';
					|en = 'Unload query result'", Язык);
	...
	Имя = НСтр("ru = 'ВыгрузитьКолонкуЗапроса';
				|en = 'UploadQueryColumn'", Язык);
	Подсказка = НСтр("ru = 'Выгрузить колонку результата запроса';
					|en = 'Unload query result column'", Язык);
	ШаблонТекста = НСтр("ru = 'Запрос = Новый Запрос;
                         |Запрос.Текст = ""$0"";
	...
	// ----------------
	Имя = НСтр("ru = 'ИзменитьДанныеОбъекта';
				|en = 'ModifyObjectData'", Язык);
	Подсказка = НСтр("ru = 'Изменить данные объекта и записать';
					|en = 'Change object data and write'", Язык);
	ШаблонТекста = НСтр("ru = 'Объект = Строка.Поле.ПолучитьОбъект();
                         |//Объект.ОбменДанными.Загрузка = Истина;
                         |//Объект.ДополнительныеСвойства.Вставить(""%1"");
                         |//Объект.ДополнительныеСвойства.Вставить(""%2"");
                         |//Объект.ИмяПоля = $0;
                         |Объект.Записать();';
                         |en = 'Object = Row.Field.GetObject();
                         |//Object.DataExchange.Load = True;
                         |//Object.AdditionalProperties.Insert(""%1"");
                         |//Object.AdditionalProperties.Insert(""%2"");
                         |//Object.FieldName = $0;
                         |Object.Write();'", Язык);
	Текст = СтрШаблон(ШаблонТекста, ИТК_ПараметрыЗаписиКлиентСервер.ИмяПараметраОтключитьМеханизмРегистрации(), 
									ИТК_ПараметрыЗаписиКлиентСервер.ИмяПараметраПропуститьПроверкуЗапретаИзменения());
...
Функция ПодсказкаПоШаблонам(Язык) Экспорт
	...
	Если ЗначениеЗаполнено(ОписаниеШаблонов) Тогда
		...
		Результат = Символы.ПС + НСтр("ru = 'Шаблоны:';
										|en = 'Templates:'") + Символы.ПС;
...
Функция НомерСимволаПоКурсору(Текст, НомерСтроки, НомерКолонки) Экспорт
	...
	Если СтрНайти(Текст, Символы.ВК + Символы.ПС) Тогда
		ПереносСтроки = Символы.ВК + Символы.ПС;
	КонецЕсли;
...
Функция ПодсказкаИспользуемойПеременной(Имя, ПредставлениеЗначения, ПредставлениеТипа) Экспорт
	
	Шаблон = "**%1 (%2)**";
	Шаблон = Шаблон + Символы.ПС + Символы.ВК + "%3";
	Шаблон = Шаблон + Символы.ПС + Символы.ВК + "*" + НСтр("ru = 'Значение переменной после выполнения'; en = 'The value of the variable after executing'") + "*";
	
	Возврат СтрШаблон(Шаблон, Имя, ПредставлениеТипа, ПредставлениеЗначения);
...
Функция ТемаКонфигуратор() Экспорт
	
	Возврат "bsl-white";
	
КонецФункции

Функция ТемаЕДТ() Экспорт
