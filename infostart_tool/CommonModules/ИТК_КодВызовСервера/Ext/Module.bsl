...
Функция ТаблицаЗначений(Адрес, Имя) Экспорт
	...
	КодНаВстроенномЯзыке = ИТК_Код.Значение(ТаблицаЗначений, , ДополнениеРезультата);
	Идентификатор = ИТК_РедакторКодаКлиентСервер.ИмяПеременнойВозвратаРезультата(ИТК_Общий.КодЯзыкаПрограммирования());
...
Функция ЗначениеПараметра(Значение, Имя = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Структура") Тогда // Граница
		...
		ЗначениеПараметра = ИТК_Запрос.ГраницаИзОписания(Значение);
	...
	КодНаВстроенномЯзыке = ИТК_Код.Значение(ЗначениеПараметра, , ДополнениеРезультата);
	Идентификатор = ИТК_РедакторКодаКлиентСервер.ИмяПеременнойВозвратаРезультата(ИТК_Общий.КодЯзыкаПрограммирования());
	...
	Возврат ИТК_ЗапросКлиентСервер.ЗначениеСложногоПараметра(ИТК_Перечисления.СложныйПараметрЗапросаВыражение(), КодНаВстроенномЯзыке);
...
Функция ТаблицаЗначенийИзВыражения(Алгоритм, УникальныйИдентификатор) Экспорт
	...
	Попытка
		...
		Результат = ИТК_Общий.ВычислитьРезультатВыражениеВБезопасномРежиме(Алгоритм);
	...
	Возврат ИТК_ВременноеХранилище.Поместить(Результат, , УникальныйИдентификатор);
...
Функция ПараметрИзВыражения(Алгоритм, УникальныйИдентификатор, ТекстОшибки = "") Экспорт
	...
	Если ТипЗнч(Результат) = Тип("ТаблицаЗначений") Тогда 
		...
		Результат = ИТК_ЗапросКлиентСервер.ЗначениеСложногоПараметра(ИТК_Перечисления.СложныйПараметрЗапросаТаблицаЗначений(), АдресЗначения);
