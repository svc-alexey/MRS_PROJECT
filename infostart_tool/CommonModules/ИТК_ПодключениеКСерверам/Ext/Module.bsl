...
Функция ИспользованиеЛицензий() Экспорт
	...
	Результат.Колонки.Добавить("Кластер", ИТК_ТипыКлиентСервер.ОписаниеСтрока(100));
	Результат.Колонки.Добавить("База", ИТК_ТипыКлиентСервер.ОписаниеСтрока(100));
	Результат.Колонки.Добавить("Лицензия", ИТК_ТипыКлиентСервер.ОписаниеСтрока(100));
	Результат.Колонки.Добавить("Сеанс", ИТК_ТипыКлиентСервер.ОписаниеСтрока(100));
	Результат.Колонки.Добавить("ИдентификаторСеанса", ИТК_ТипыКлиентСервер.ОписаниеУникальныйИдентификатор());
	...
	Результат.Колонки.Добавить("Лицензий", ИТК_ТипыКлиентСервер.ОписаниеЧисло(10));
...
Функция ПроверкаПодключения(Параметры)
	...
	Если Параметры.Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияCOM()
			И ИТК_ОбщийКлиентСервер.ИнформационнаяБазаФайловая() Тогда
			
		Возврат НСтр("ru = 'Недоступно в файловой базе';
					|en = 'Not available in the file database'");
			
	КонецЕсли;
	
	Попытка
			
		ОткрытьСоединение(Параметры);
...
Процедура ИспользованиеЛицензийПоКластеру(Результат, ОписаниеПодключения)
	...
	Если Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияCOM() Тогда
		
		ЗаполнитьИспользованиеЛицензийCOM(Результат, Соединение, ОписаниеПодключения);
		
	ИначеЕсли Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияRAS() Тогда
...
Функция ОткрытьСоединение(Параметры)
	...
	Если Параметры.Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияCOM() Тогда
		
		Соединитель = Новый COMОбъект("V83.COMConnector");
		Результат = Соединитель.ConnectAgent(Параметры.Сервер);
		
	ИначеЕсли Параметры.Подключение = ИТК_ПодключениеКСерверамКлиентСервер.ТипПодключенияRAS() Тогда
