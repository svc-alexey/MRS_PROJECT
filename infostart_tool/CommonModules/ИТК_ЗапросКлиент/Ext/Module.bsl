...
Процедура ОткрытьПреобразоватьВоВложенный(Форма, Элемент, ЗаголовокПредупреждения, ОписаниеОповещенияОЗавершении = Неопределено) Экспорт
	...
	Заголовок = НСтр("ru = 'Введите идентификатор для вложенного запроса';
					|en = 'Enter id for subquery'");
	Режим = ИТК_Перечисления.РежимРедактированияИдентификатораИдентификатор();
	ИдентификаторПоУмолчанию = НСтр("ru = 'ВложенныйЗапрос';
									|en = 'SubQuery'");
	...
	РезультатПреобразования = ИТК_Запрос.ПреобразоватьТекстЗапросаВоВложенный(ВыделенныйТекст, ИдентификаторПоУмолчанию);
...
Процедура ВводИдентификатораВложенногоЗапросаЗавершен(Результат, ДополнительныеПараметры) Экспорт
	...
	ВыделенныйТекст = ИТК_РедакторКодаКлиент.ВыделенныйТекст(Элемент);
	РезультатПреобразования = ИТК_Запрос.ПреобразоватьТекстЗапросаВоВложенный(ВыделенныйТекст, Результат);
...
Процедура ОткрытьКонструктор(Знач Текст, Оповещение, РежимКомпоновкиДанных = Ложь) Экспорт
	...
	Исключение
		...
		ЧастиТекстаВопроса.Добавить(НСтр("ru = 'В запросе содержатся ошибки:';
										|en = 'The request contains errors:'"));
		...
		ЧастиТекстаВопроса.Добавить(Символы.ПС);
		ЧастиТекстаВопроса.Добавить(НСтр("ru = 'текст запроса будет потерян. Продолжить?';
										|en = 'The query text will be lost. Continue?'"));
		ТекстВопроса = Новый ФорматированнаяСтрока(ЧастиТекстаВопроса);
			
		ЗаголовокВопроса = НСтр("ru = 'Запуск конструктора запроса';
								|en = 'Run the query constructor'");
