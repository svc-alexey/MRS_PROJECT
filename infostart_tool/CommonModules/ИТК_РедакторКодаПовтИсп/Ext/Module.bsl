...
Функция ОписаниеОбъектаМетаданных(ИмяОбъекта) Экспорт
	...
	Если ИТК_МетаданныеКлиентСерверПовтИсп.ЭтоВнешнийИсточникДанных(ИмяОбъектаКоллекции) Тогда
		
		ОписаниеТаблиц = Новый Структура;
		ОписаниеКубов = Новый Структура;
		ОписаниеТаблицЭлементы = Новый Структура;
		
		ОписаниеТаблицы = Новый Структура;
		Для Каждого Таблица Из ОбъектМетаданных.Таблицы Цикл
			
			Если Таблица.ТипДанныхТаблицы = Метаданные.СвойстваОбъектов.ТипДанныхТаблицыВнешнегоИсточникаДанных.ОбъектныеДанные Тогда
				ТипДанныхТаблицы = "ObjectData";
			Иначе
				ТипДанныхТаблицы = "NonobjectData";
			КонецЕсли;
			ОписаниеТаблицы.Вставить("tableDataType", ТипДанныхТаблицы);
			
			ОписаниеПолей = Новый Структура;
			Для Каждого Поле Из Таблица.Поля Цикл 
				ОписаниеПолей.Вставить(Поле.Имя, ИТК_РедакторКода.ОписаниеПоля(Кэш, Поле));
			КонецЦикла;
			ОписаниеТаблицы.Вставить("properties", ОписаниеПолей);
			
			ОписаниеТаблицЭлементы.Вставить(Таблица.Имя, ОписаниеТаблицы);

		КонецЦикла;
		
		ОписаниеТаблиц.Вставить("items", ОписаниеТаблицЭлементы);
		ОписаниеОбъекта.Вставить("tables", ОписаниеТаблиц);
		
		ОписаниеОбъекта.Вставить("cubes", ОписаниеКубов); 
		
		Результат = ОписаниеОбъектаМетаданныхРезультат(ИмяКоллекцииMonaco, ОбъектМетаданных, ОписаниеОбъекта);
		Возврат Результат;
...
Функция ОписаниеОбъектаМетаданныхРезультат(ИмяКоллекцииMonaco, ОбъектМетаданных, ОписаниеОбъекта)
